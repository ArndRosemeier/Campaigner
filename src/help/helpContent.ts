...
export const HELP_CONTENT: Record<HelpTopic, HelpEntry> = {
  ...
  module: {
    ...
    tips: [
      ...
      'If a step fails (for example an LLM error), the forge does not restart from scratch: "Retry failed step" re-runs only that step, and everything generated before it is kept and used as context. The failed run stays in the Runs tab with its error message.',
