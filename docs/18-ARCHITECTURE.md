...
| **Favourite a gallery image — ONE field, ONE reading, ONE update seam** (owner request, docs/17 row 366; the owner's own scope: *"Just a sorting mechanism, favourites stay on top. Fresh favourites sort above earlier."*) | `domain/image.storedImageSchema.favouritedAt` (nullish, additive — a row written before it carries no key, and raw Dexie reads are not parsed, so BOTH `null` and `undefined` mean not-favourited) is the WHOLE state: non-null means favourited and the VALUE is the order. `domain/image.isImageFavourite(image)` is THE one reading of the flag and `domain/image.sortGalleryImages(images)` THE one order — favourites first by `favouritedAt` DESC (a stable sort, so two stamped in the same millisecond keep the gallery's own order), then every non-favourite in the INPUT order (the artifact's own `imageIds`), and an empty favourite set returns that input order untouched. `features/campaign/components/images-section.ImagesSection` maps the sorted list and renders one star per image (a SIBLING of the open-image button, `aria-pressed`, an id-suffixed testid); the write is `imageRepo.setImageFavourited(id, favourited)` — stamps `Date.now()`, clears back to `null` — riding the ONE private `updateImageRow` (`db.images.update`, which `setImageRole` also calls). `tests/architecture/one-image-row-write.test.ts` is the exactly-one SOURCE SCAN over `db.images.update(` | a SECOND boolean beside the timestamp (two truths that drift); a component-side or per-surface row write; a sort that re-orders the NON-favourites (flag-off must be the pre-change order — the slice's strongest pin); a new Dexie table/index/version or a global store for a display sort; letting the field reach a PICK, delete/cleanup, print or cache path (it is a sort, docs/17 row 366) |
...
| **Review ONE module text against the owner's four criteria and, only if it finds something, rewrite it — THE one adversarial pass** (docs/17 rows 352/353/356; **TWO triggers are WIRED: the AUTOMATIC one — docs/17 row 358: pass 0's premise first, then each part as it is written, both behind the module row's `adversarialGeneration` flag (OFF by default) — and the CHAT one on the owner's own requirement (docs/17 row 360, verbatim: *"This step can be automated to run once, but it should also be triggerable in the module chat."*)**) | `llm/adversarialPass.runAdversarialPass({ moduleId, target, text, signal })` where `target` is `{ kind: 'premise' }` or `{ kind: 'part', planIndex }`: (1) the durable snapshot through the ONE `db/moduleVersionRepo.snapshotModuleVersion` seam, BEFORE anything; (2) ONE zod-validated critique call on the module-generation pattern (`getSettings().defaultChatModel` + `schemaResponseFormat('adversarial-critique', …)` + `defaultReasoningEffort`, never a persona/runEngine) whose `issues[].kind` enum is EXACTLY `inconsistency` \| `motivation` \| `fun` \| `originality` (with `severity`, the critic's `message` and a `where` locator); (3) ONLY when `issues.length > 0`, the EDITOR `llm/canvasRefine.transformModuleText` — the ONE text-transform core, target kind `premise`/`part` — with the findings as a structured instruction; (4) a REPORT (`originalText`, `snapshot`, `critique`, `edit \| null`) and NO write. ADVISORY: findings never throw and never block, an empty `issues` array is a quiet success that does NOT call the editor, and only a BOUNDARY failure (malformed critique or editor reply) throws loudly. Its callers are EXACTLY two files, both of them legitimate callers and counted by the source pin: `llm/moduleGen` at two flag-gated sites (`adversarialReviewEnabled`; the pin counts both call sites and the guard), and `features/modules/canvas/chatChanges.executeAdversarialChange` at ONE site — the chat trigger of docs/17 row 360, which runs the SAME pass for the premise or a named part and renders the critique's findings on the chat's outcome card. Every edit rides an EXISTING write seam — `features/modules/partText.saveModulePartText` for a part in generation, `db/moduleRepo.patchModuleSpine` (the row-357 atomic subfield seam) for the premise in both lanes, and the canvas chat's OWN `features/modules/canvas/chatApply.applyChatCommands` for a part in the chat (the same ladder the `<edit>` commands ride; the turn then persists it through the ONE split-save `saveWholeModuleDocument`) — so the parts-document scaffolding stays DERIVED, never hand-assembled | a second editor prompt or reply schema; a persona or a run-engine step; a fifth criterion, a score/ratio/cardinality; a prose GATE (the class `features/modules/module-problems` deliberately excludes); a new busy mechanism (the canvas guard stays in `refineModuleText`); a second snapshot seam; a write from the pass (the caller writes); a second part-text write seam; a trigger site outside the two sanctioned callers |
...
| **Change an artifact from the CHAT — and run the adversarial review** (docs/17 row 104 — the write half behind the read half above, so it supersedes that row's "the write half … does NOT exist here" parenthetical; the adversarial variant added by docs/17 row 360) | `llm/canvasChat`'s change half + `features/modules/canvas/chatChanges`: the assistant may emit `<change operation="repopulate\|everything"><name>EXACT NAME</name><instruction>…</instruction></change>` (≤ `MAX_CHANGES_PER_REPLY` = 3 per reply), parsed by the SAME `parseCanvasChatReply` strict extractor in the SAME one left-to-right walk as `<edit>`/`<request>` — a malformed block (unknown attribute, duplicated or invented `operation`, wrong children, empty name/instruction, unterminated) or an over-cap reply fails the WHOLE reply with NOTHING executed. Names resolve through the SAME `lib/wikilinks.resolveWikiLink(name, pool, { moduleId })` the chips and the read half use (`llm/canvasChat.resolveChatArtifactName`, ONE `loadChatDetailsPool` per turn); an unresolved/ambiguous name is a NAMED refusal carrying the resolver's own candidates — the app never guesses which row to OVERWRITE. `operation` is REQUIRED for an encounter and refused by name for every other kind, and there is NO default (`repopulate` restocks, prose untouched; `everything` regenerates the encounter with its prose, and never sets the editor's own `redesignProse` checkbox). Each change runs SEQUENTIALLY (one at a time, in reply order) through the ONE seam (`features/modules/change-artifact.changeArtifact`, row 101) with the resolved row's id + the instruction + the operation; the CHAT PATH itself writes NO row. Since each change is a real generation, the turn HANDS THE MODULE SLOT OVER for the phase (`llm/canvasBusy` is not re-entrant — §4) and a slot another generation holds is a named `MODULE BUSY` outcome, never a silent skip. The outcomes (`changed` / `refused` / `unsupported` / `unresolved` / `ambiguous` / `busy` / `failed`) ride the SAME one follow-up call the read half established, as a `<change-results>` block (`renderChangeResults` + `CHANGE_RESULTS_HEADER` + `CANVAS_CHAT_CHANGES_INSTRUCTION` through `buildCanvasChatFollowUpPayload`; every non-applied verdict reads `NOT APPLIED: <VERDICT>` with the asked-for instruction echoed), and a `<change>` in THAT reply is `ignoredChanges` — never a third call. The OWNER sees each outcome the moment it settles (`chatChanges.reportChatChangeOutcome`: a loud toast naming the artifact, its kind, the operation and the instruction, plus a progress-dock job `Changing «<name>»` linked to the row; the `failed` copy never claims nothing changed) in BOTH flows (`chatController` + `snapshotChat`). Recovery invents nothing: `updateArtifact` recorded a NEW revision with the previous one intact, restorable from the artifact editor's existing revision list (`restoreRevision`). **THE ADVERSARIAL VARIANT (docs/17 row 360, the owner's own requirement: *"This step can be automated to run once, but it should also be triggerable in the module chat."*)** rides the SAME `<change>` tag and the SAME strict extractor, with NO name and NO instruction body: `<change adversarial="premise"></change>`, or `<change adversarial="part" part="N"></change>` where N is the 1-based part number the parts document labels (`canvasChatAdversarialTargetSchema` → the parsed command is `{ adversarial: { kind: 'premise' } }` or `{ adversarial: { kind: 'part', planIndex } }`, and mixing it with `operation` or carrying a body fails the WHOLE reply). The executor resolves the target's CURRENT text — the LIVE per-part snapshot the model was shown for a part (`CanvasChatChangeContext.parts`), the row's own premise for the premise — and calls the ONE `llm/adversarialPass.runAdversarialPass` (its only chat caller; the pass is untouched and takes the durable snapshot itself). `edit === null` is the quiet `clean` outcome: NOTHING is written and the outcome says so. An edit is applied through the target's EXISTING path — a PART through the chat's OWN `chatApply.applyChatCommands` on the turn's live document handle, which the turn then persists through the ONE split-save (`saveWholeModuleDocument`); the PREMISE through `db/moduleRepo.patchModuleSpine` + the promote scan — so the change is undoable from the EXISTING version stack (the pass's snapshot, premise included since row 357) and the canvas doc can never drift from the row. THE FINDINGS ARE THE POINT and they are rendered by the chat's OWN card: `CanvasChatChangeOutcome.findings` carries one rendered line per issue (`llm/canvasChat.adversarialFindingLine`: kind, severity, the critic's message, where), the `<change-results>` block lists them for the model, `reportChatChangeOutcome` toasts them, and `ChatSidebar`'s ONE `OutcomeCard` shows them above the edit's before→after (a `clean` card carries the quiet "nothing to fix" instead) | a chat-side or per-surface row writer (no provenance, no per-kind brief, no creature-row guard, no busy gate); a second command tag vocabulary, or a regex-guessed block; a default encounter operation; a silent newest-wins pick on an ambiguous name; running the changes in parallel; a second follow-up call; `catch`-and-continue around a change outcome (a specialist failure is a named `failed` outcome the model reads AND a loud owner toast); reading an abort as a change; a new rollback/undo mechanism beside the revision list; a second chat action path, command tag or busy mechanism for the review (it is the SAME `<change>` seam and the SAME turn); a new overlay/dialog/card component for the findings; a second critique/editor/snapshot seam; a review that writes when the critique found nothing |
...
| Snapshot the whole document BEFORE an AI change (durable simple undo, owner-directed, ledger 63; **the premise half added, ledger 357**) | `db/moduleVersionRepo.snapshotModuleVersion(moduleId, source, label)` — ONE row `{id, moduleId, createdAt, source, label, docText, premise}` in the additive `moduleVersions` table (schema v19, no upgrade fn), where `docText` is the WHOLE parts document BYTE-EXACT in the ONE `assembleModulePartsDocument`/`splitPartsDocument` format and `premise` is the spine premise BYTE-EXACT beside it (`string \| null`, additive `.default(null)` — NO index, NO migration; a row written before the field parses as `null` = NOT CAPTURED and restores exactly what it always restored, leaving the premise as it stands, while a captured EMPTY premise is `''`; the premise is deliberately NOT folded into `docText`, which would be a second document format the split seam cannot read); called immediately BEFORE the write, never after, by: `saveWholeModuleDocument` with `origin: 'ai'` (editor chat batch, preview-snapshot chat batch, accepted Refine/Rewrite proposal, restore — the `version` arg is REQUIRED for AI saves and its absence THROWS, writing nothing; **ONE snapshot per AI SAVE, never per part and never twice** — the whole batch shares it, because two snapshots split one revision in two, and the count is pinned on both chat surfaces in `tests/features/canvas-chat-turn-parity.test.ts`), `runParts` at entry (generation / missing-part fill / single-part rewrite+regenerate / board staged rewrite / floor repairs), each `normalizeModuleEntityNames` pass, and `entity-panel.applyProposals` (consented rewrite apply); `createdAt` is strictly increasing per module; `MODULE_VERSION_CAP` 25 per module, oldest pruned in the INSERT transaction, retention stated in the menu (**restore** validates the snapshot against the CURRENT part plan and refuses loudly on a mismatch, then rides the shared whole-doc proposal → accept → the split-save, taking its own pre-restore snapshot, and puts BOTH halves back: the parts through the split-save and the stored premise FIRST, before any part write, through the ONE spine-subfield seam `moduleRepo.patchModuleSpine` — a premise that cannot be applied throws `ModuleVersionPremiseError` and aborts the whole restore with NO part written, toasted as "its saved premise could not be put back, so nothing was restored", so a half restore is never presented as a success; a version row predating the field carries `null` and leaves the premise as it stands; the menu's per-entry note (`moduleVersionPremiseNote`) previews a captured premise, reads a captured empty one as `(empty)` and SAYS when an older entry captured none; **clear all previous versions** = destructive-confirmed menu item, module-keyed, no snapshot first, document untouched) | a second document format (the premise is NEVER folded into `docText`) or a premise-only history/restore path; a premise restore AFTER the parts (a partial that survives a failure); snapshotting AFTER the change; snapshotting manual typing (CM6 history owns it) or the board's Apply/Discard of a staged rewrite (already captured at pass entry); a silent cap; a side-door restore write; clearing another module's stack; letting Clear chat delete undo history; making the session ledger durable |
...
| Return ONE surface's canvas chat to a pristine state (Clear chat; docs/17 row 362 for GM assist) | `features/modules/canvas/clearChat.clearModuleChat({ moduleId, key, framing })` — the `framing` says WHICH surface is cleared and `canvasChatThreadPersists(framing)` is the ONE rule that only the module thread has a row half and a ledger half (the GM-assist clear drops its store key and nothing else). ROW FIRST (awaited: `chatPersist.clearPersistedChatThread` → the same `patchModule({chatThread: []})` seam the debounced writer uses, cancelling that key's pending debounce; a failure cancels the whole action), then the live conversation (`chatStore.clearModule`) + that module's session ledger (`canvasStore.clearModule`, owner-keyed `moduleId#planIndex`), the page drops the `lastReplacement` highlight (both surfaces); the panel-header control confirms through a destructive AlertDialog whose copy states what is NOT cleared; refuses LOUDLY (toast, nothing cleared) while a reply is in flight or any canvas AI action is live for the module. **THIS IS THE CONVERSATION REMOVAL (docs/17 row 367, MEASURED — the owner's "Chats should also be removable", granularity "Delete a whole conversation", has NO second action).** Read end to end: the module surface's clear already deletes the whole conversation, not merely the screen — the saved thread on the row reads `[]` (absence, asserted), a marker written into both halves of a thread and then removed survives in NO Dexie table and not in `localStorage`, and the module's DURABLE `moduleVersions` record of the edit that thread applied survives byte for byte (module history is not the conversation). GM assist holds only a SESSION thread until its persistence slice (docs/20 QUEUE row 363), so there is no saved copy of it to delete and its clear drops its store key and nothing else. Pins: `tests/features/canvas-chat-clear.test.tsx` (cancelling deletes NOTHING; a module removal leaves the GM-assist thread BYTE-IDENTICAL; the marker scan; the durable record and the document unchanged) and `tests/architecture/one-canvas-chat-framing.test.ts` (ONE writer of the row's `chatThread`, ONE removal entry point) | clearing document text (not an undo — revert lives in the Versions menu: the DURABLE snapshot stack, plus the session ledger as session review state); clearing the module's DURABLE versions (undo history is not chat state — the menu's own destructive item owns that); a second persistence path for `chatThread`; **a GM-assist clear that empties the shared session ledger or the module thread's half** (the ledger cannot tell the two surfaces' undo apart); touching another module's thread/ledger/versions/highlight; a cancel-then-clear path |
