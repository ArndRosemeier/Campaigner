...
- **CLOSED (docs/17 row 259) — family E's BATTLE-ROW writer IS repointed: the last runtime
  ...
  **STILL NAMED, and NOT this seam's business (two live library reads outside the declared set):**
  (a) `battle.encounterArtifactId` is a reference to the SEEDING encounter artifact, and a
  LIBRARY-scoped encounter IS runnable (`features/campaign/components/artifact-editor.tsx`'s
  `EncounterRunAction` opens the module picker for `moduleId === null`, then `run-battle-seed`)
  while `BattleSurface` re-reads it with `getAnyArtifact` — so a battle seeded from a global
  encounter keeps a provenance/spawn-source dependency on the library row; adoption deliberately
  does not repoint it because the battle is keyed BY that id (`getBattleByEncounter`) and repointing
  it would change the battle's identity (a second board for the same encounter). (b) the E1 render
  divergence for a reference adoption CANNOT reach — a gone library row, or an `import-anyway`
  landing — where `db/monsterResolve` passes the campaign-only `getArtifact` while every other
  reader passes `getAnyArtifact` (`db/battleSeed`, `db/mob-portrait-queue`, `db/creatureRepo`);
  that is board row 258 and needs its own decision (one getter for the reference, or a declared
  campaign-only contract), not a silent absorption here. The `links[]`-not-remapped-on-import gap is
  the SAME class and is board row 256. (c) `db/battleRepo.scrubArtifactFromBattles` scrubs
  `board.tokens` but NOT `board.stage.tokens`, so deleting an OWNED artifact can leave a stage
  snapshot token dangling (the row-259 seam NAMES it at the next upgrade/retry, but the scrub itself
  is still one carrier short).
