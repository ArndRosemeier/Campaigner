...
- **The canvas ledger is session-only by design.** The per-part version
  ...
  Consequence (known behavior, not a bug): the CHAT THREAD persists on the
  module row, so a canvas reopened on a previous session's thread can show
  applied edits while Versions reads "Nothing accepted yet" and Save stays
  disabled (the doc matches the row). The chat's **Clear chat** control
  (`clearChat.clearModuleChat`, §2.3) is the way back to a pristine state for
  one module.
