...
- **CLOSED (docs/17 row 259) — family E's BATTLE-ROW writer IS repointed: the last runtime
  ...
  **REVERSED AND CLOSED (docs/17 row 268) — the seeding encounter is ADOPTED, not excepted.**
  ...
  both, Dexie **v29** backfills (v28 already ran and a landed upgrade body never re-runs), the start-up
  retry heals, `BattleSurface` reads the encounter with the CAMPAIGN-SCOPED `getArtifact` (no library id
...
- **OWED (docs/17 row 268 completeness sweep) — EVERY remaining stored library identifier, named with
  ...
  2. **`board.mapImageId` / `board.stage.mapImageId`** (`domain/battle.ts:211` / `:197`) **and
     ...
     NAMES a gone one), backfilled by Dexie v30, with reachability PROVEN through the real seed
  ...
  8. **THE ENTRY POINTS THAT CAN RE-INTRODUCE ONE.** **CLOSED by docs/17 row 256 for the IMPORT
     ...
     `lib/backup.ts` writes rows into Dexie directly — so a whole-DB backup restored over the current
     schema still lands raw library keys; that is exactly what the start-up RETRY is for, and it is
     PROVEN to heal a re-landed battle key (`tests/db/libraryAdopt.test.ts`, the retry arm) because it
     runs the SAME seam, which collects `encounterArtifactId` and its re-seed stamp. The gate that
     decides whether the retry RUNS is `settings.libraryAdopt.unresolved`, and a restored backup
     restores its own journal from the same snapshot — so a backup taken while references were still
     unresolved restores the worklist WITH the keys, and one taken after they were all adopted
     restores none (the journal was cleared with them). Those are the only two states a backup can
     hold, because every adoption that changed data also persisted the worklist beside it in the same
     transaction. The one arm whose rawness is NOT visible in that journal is the ordinary campaign
     IMPORT — which is why the import path adopts eagerly in its own call rather than leaning on the
     retry.
...
- **CORRECTED (docs/17 rows 255b, then 269) — `monsterEntry.originToken` is INERT on SOME paths
  ...
  **PER PATH, THE WHOLE TRUTH — this is the form the claim must take from here on.**
  ...
  an UNCONVERTED `rulebook` citation, an `npc-ref` still carrying `creatureRef` — the loud
  legacy/failure arm, whose missing chunk is still named — and a token-bearing row with no copied
  block of its own, which keeps the citation read because it has no bytes to ground on (a
  placeholder is forbidden, AGENTS rule 1).
