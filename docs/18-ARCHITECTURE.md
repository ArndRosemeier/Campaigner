...
| **Copy a library creature's stats onto a module row AT WRITE TIME — the ONE copy operation, shared by the migration's backfill and every live mint** (docs/17 row 255a) | `domain/libraryCopy.copyCreatureStats(citation, fallbackName, lookups)` is THE one copy: PURE, tx-callable, injected lookups; it returns the library `statBlock`, the STAMPED `sourceLine` (`creatureOriginLabel`, which used to be composed at read time) and the opaque `originToken` (`libraryCreatureKey`), or a NAMED unresolved reason. `db/libraryCopy.copyCreatureStatsFromDb` is the ONE live wrapper (over `db/creatureRepo.creatureLookups`, so a copy and a resolution cannot disagree about the content-hash fallback). The Dexie v24 backfill (`db/mobCopyRepair`) calls the SAME seam with its tx tables, and the live WRITE paths call the wrapper: `llm/runEngine.rosterMobCopyFor` (encounter finalize), `features/campaign/components/monster-source` (the editor picker), `features/play/battle/spawn-picker-logic.buildMobPickEntry` (battle spawn). A vanished chunk is a LOUD refusal (`domain/libraryCopy.creatureCopyRefusal`) — a write path never mints a pointer as a consolation. `tests/architecture/one-library-copy.test.ts` is the exactly-one pin (the token is minted only by the identity layer + the seam; the copy operation is DEFINED once; no write path composes an origin label); `tests/features/mob-copy-on-write.test.ts` is the DIFFERENTIAL pin (the pure seam, the generator and the picker produce ONE identical copy) and proves the backfill equals a fresh write-path copy | a SECOND copy mechanism at a write site (the fragmentation this arc exists to remove); composing `creatureOriginLabel` or `contentIdentityFor` at a write path (the stamped line comes off the seam); minting a `rulebook`/uuid-only source when the chunk cannot be copied (refuse loudly instead); calling a `db`-bound wrapper inside the upgrade tx |
...
- **STILL OPEN — the pointer ARMS are not deleted, the module arm is not converted, and
  ...
  **NOT YET CONVERTED, named so a successor can be briefed from here:** (i) the NPC CAST
  path — `db/creatureRepo.castCreatureAsNpc`, the module generator's ONLY cast path, still
  writes `creatureRef`, so every cast NPC (and therefore every module `entityKinds[].bestiary`
  FULFILMENT, which is that cast row) is still a pointer; its conversion needs a reuse
  identity that survives the dropped ref (the stored `originToken`/`sourceLine`) and the
  ~10 cast-path test files rewritten; (ii) the BATTLE CARD read chain (item 4 below) — a
  copied mob's seeded token now freezes its `chunk:` identity onto the seed row
  (`db/battleSeed`), but the card's stat block still comes from `tokenCreature`'s live
  library read; (iii) family C SPELLS (a stat block's spell NAMES are still resolved against
  the library) — the owner's decision is to copy the full entry with its `publication
  {title, license}` line, and its export-size cost is UNMEASURED because that slice did not
  land. TWO of the three blockers have LANDED. The
