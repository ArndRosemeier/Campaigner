...
| Delete a module / campaign / artifact | `moduleRepo.deleteModule` ('cascade' \| 'keep' \| 'promote-referenced') / `campaignRepo.deleteCampaign` / `artifactRepo.deleteArtifact` — 'promote-referenced' adopts outside-referenced rows (fresh `modulesReferencingOwnedArtifacts` scan: wiki-graph edges + roster + battle tokens) BEFORE the tx, then cascades the rest; the list dialog shows the third state with the referenced names. `deleteModule` sweeps the module's durable version rows in the SAME tx (`db.moduleVersions` by `moduleId` — they describe a document that no longer exists) and frees its own cover blob after the row delete (captured before, `deleteImageIfUnreferenced` after — the refcheck's cache-table read cannot join the delete scope); `deleteCampaign`'s image sweep frees module + campaign covers with everything else | ad-hoc cascades — these are transactional, recount-honest (rows re-listed inside the tx), scrub battles/links/images |
...
- **The canvas ledger is session-only by design.** The per-part version
  ...
  group reads "Nothing accepted yet" and Save stays disabled (the doc matches
  the row) — the durable group above it lists that session's AI pre-change
...
- **Campaign-level wipes leave durable module versions orphaned** (accepted,
  ledger 63): `moduleRepo.deleteModule` sweeps a deleted module's
  `moduleVersions` rows in its own tx, but the campaign-scoped wipes
  (`deleteCampaignWorkspace`, `removeAllGeneratedContent`) do NOT — they are
  the owner's untouched surfaces in this arc, so their version rows survive
  their modules. Bounded (≤ `MODULE_VERSION_CAP` per module), invisible
  (nothing lists versions for a module that no longer exists) and swept by
  nothing today; a future campaign-level sweep may add them, deliberately not
  here.
