...
- **OWED (docs/17 row 268 completeness sweep) — EVERY remaining stored library identifier, named with
  ...
  2. **`board.mapImageId` / `board.stage.mapImageId`** (`domain/battle.ts:211` / `:197`) can name a
     LIBRARY IMAGE id: `db/battleSeed.resolveMapImageId`'s linked-LOCATION fallback (`:112-116`) returns
     the linked location's cover, and the adoption holders collect ARTIFACT ids only
     (`domain/libraryAdopt.ts:132-142`). The reachable case is a pre-v29 battle seeded from a library
  ...
  8. **THE ENTRY POINTS THAT CAN RE-INTRODUCE ONE.** `lib/exportImport.ts:902-911` remaps a battle's
     ...
     backup restored over the current schema (`lib/backup.ts`), can land a library key on a battle that
     no Dexie version will re-key. The import path's library references are the class already recorded
     as board row 256 (`links[]` not remapped on import); this is the battle-key member of it, NAMED
