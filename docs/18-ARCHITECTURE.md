...
- **KNOWN DEBT (docs/17 row 340) — the shared `DialogContent` cap wins the cascade over every
  ...
  `twMerge(base, 'max-h-[80vh] overflow-y-auto')` keeps the base's `@supports` cap, so the caller's
  80vh intent is dead on any browser that knows `svh` (all modern ones). This is PRE-EXISTING (row
