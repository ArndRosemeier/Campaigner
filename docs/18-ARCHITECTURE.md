...
| Land a DEBOUNCED row write when the PAGE is going away — a frozen or discarded tab, which never unmounts (docs/17 row 111) | `lib/pageFlush.registerPageFlush(flush)` — ONE registration list behind ONE `pagehide` listener and ONE `visibilitychange` listener (installed only while at least one flush is registered, removed when the last one goes; the hook fires only for `hidden`, never for `visible`). Registered by TWO writers today: `features/modules/canvas/chatPersist` (at module scope — the pending queue and the debounce are the writer's own, so the writer owns its flush) and `features/modules/new-module-dialog` (`flushPendingDraft`). NOT yet registered: `features/modules/board/BoardPage`, whose 600 ms layout debounce still flushes on unmount only and has the same exposure — wiring it is a pending-gated flush plus one `registerPageFlush` call (ledger 111 (4b)). Every registered flush MUST keep the seam's three-part contract: PENDING-GATED (nothing queued ⇒ nothing written), IDEMPOTENT (the pending work leaves the writer's queue before the write, so `hidden` then `pagehide` is ONE write) and NON-THROWING (the writers already toast their own failures; anything escaping a lifecycle listener is toasted by the seam). `registerPageFlush` returns the unregister function, which is the `useEffect` cleanup for a component-scoped writer | flushing on every `visibilitychange` (a tab switch is not a write — measured: removing the `hidden` gate reds the named pin); registering a writer's UNMOUNT flush when it is not pending-gated (the draft's `flush` writes a touched draft even after its timer fired — the page-hide half needs its own gated wrapper, and the ungated version was measured to write on a tab switch); a per-writer `pagehide` handler (the one-handler-per-writer shape); awaiting the write (a lifecycle handler cannot hold the page open); a Dexie version, a localStorage mirror or a timer-cancel API to make this work |
...
- **"Unmount" is not "the page is going away" — and a flush test that waits longer than the debounce proves nothing** (ledger 111). A debounced writer's unmount flush covers a route change and NOTHING else: a tab that is backgrounded and then frozen, bfcached or discarded never unmounts, so a settled chat turn or a typed New Module draft sitting inside its window was simply gone (`lib/pageFlush` is the seam that closes it for those two: `pagehide` plus `visibilitychange` → `hidden`; the module board's layout debounce is the third writer with this exposure and is deliberately not on the seam yet — ledger 111 (4b)). Two rules come out of building it, both measured on this box. (1) **A flush must be PENDING-GATED**, because `visibilitychange` fires on every tab switch, minimise and app-background: an ungated flush turns a display event into a row write — measured by removing the `hidden` gate (reds the "visible is not a write" pin) and by the draft writer, whose unmount `flush` writes a touched draft even after its timer fired, so its page-hide half had to be a gated wrapper rather than the unmount function itself. (2) **A test for a flush cannot wait longer than the debounce it is bypassing.** The first version of the chat page-hide pin asserted with a 5 s `waitFor` and stayed GREEN with `registerPageFlush` deleted outright: the 600 ms debounce landed the write inside the wait, so the test proved the timer, not the flush. Every flush pin here now caps its wait BELOW the writer's debounce (400 ms against the chat writer's 600 ms, 300 ms against the draft's 500 ms) and asserts the row is UNWRITTEN before the event, which is what makes the injection fail. Generalised: when a test's subject is "this made the work land EARLIER than it otherwise would", the assertion must be bounded by the thing it is beating.
