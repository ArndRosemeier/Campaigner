...
- **A `title` beside a `BlockedControl` is invisible, and it is exactly where a
  ...
  and says so; its known list is asserted by EQUALITY, so an offender the list
  does not name reds it and fixing a named one reds it too. Two things it cannot
  do: it reads
  `src/**` as text (a sentence COMPOSED at runtime is invisible to it), and it
