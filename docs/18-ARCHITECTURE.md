...
- **STILL OPEN — the pointer ARMS are not deleted, the module arm is not converted, and
  ...
  **BOTH write/read blockers have LANDED (docs/17 row 255b); the ONE blocker left is family C:**
  ...
  with the chunk DELETED. (iii) family C SPELLS (a stat block's spell NAMES are still resolved
  against the library) — the owner's decision is to copy the full entry with its `publication
  {title, license}` line, and its export-size cost is UNMEASURED because that slice (row 255c)
  has not landed. The
