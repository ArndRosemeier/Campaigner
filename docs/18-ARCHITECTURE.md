...
| **List an encounter's MOBS under its row in the reader's entity panel** (the owner's answer, verbatim: *"Keep the jump, and list the encounter's mobs below its row in the reader's entity panel."*; docs/17 row 146, docs/11 §What a roster row PRINTS) | `features/campaign/components/monster-source.MonsterStatblocksPanel` — THE one in-app roster renderer, now mounted by `features/modules/entity-panel.EntityRow` (the `entity-encounter-mobs` block below an encounter row) alongside the encounter editor (`kind-forms.EncounterForm`) and the session-mode card (`play/artifact-cards.EncounterCard`, which takes the pool as `artifacts`). Every entry is listed — `none` included, whose line is the formatter's own no-citation sentence — its REFERENCE comes from `domain/encounterResolve.rosterReferenceFor` (the `npc-ref` arm takes an explicit `targets` pool, so a surface STATES what it can cross-reference instead of claiming one) and its box from `rosterStatBlockFor(...) ?? resolved.statBlock` — the print rule with the app's one documented `npc-ref` exception (a sidebar has no page to turn to; docs/11). The row's jump into the workspace and `RunBattleButton` are untouched | a reference string composed at the reader or the panel (a source scan in `tests/features/reader-encounter-roster.test.tsx` holds `entity-panel.tsx` free of `rosterReferenceFor`/`rosterStatBlockFor`/`resolveMonsterEntr` and of the reference vocabulary itself); a second roster renderer for the reader; resolving the roster in the reader (the panel owns the ONE `db/monsterResolve` call); dropping a `none` entry (a name-only mob is still a mob the GM must find); a box under a citation nothing can resolve |
...
- **Three notation residues survive in the stored text of real fixtures** (docs/17
  row 149 — found while fixing the brace/table corruption, recorded with their
  fixture lines instead of bundled into that fix). They are the SAME class as the
  corruption row 149 repaired, and each needs a rule the seam does not have:
  1. `@Embed[Compendium.dnd5e.tables24.RollTable.dmgBagOfBeansEff rollable
     ...
     stores `dmgBagOfBeansEff rollable caption=false` — a THIRD dnd5e notation
     (`@Embed`, with a space-separated argument list inside the brackets) that
     the shared `@`-notation rule resolves by its last dotted segment.
  2. `&Reference[prone]` (`tests/fixtures/packs/dnd5e/saber-toothed-tiger.yml`)
     survives VERBATIM, because the dnd5e prelude's `&reference[…]` rule is
     case-SENSITIVE and the corpus spells it `&Reference`. One `i` flag away —
     but a rule change, not a declaration change, so it is not bundled.
  3. `@Damage[(ceil(@item.level/2))[persistent,acid]]`
     (`tests/fixtures/packs/pf2e-rules/acid-splash.json`) stores
     `level/2))[persistent,acid]` — NESTED brackets defeat the `@`-notation
     rule, which stops at the first `]`.
  Each is pinned as CURRENT behaviour in `tests/ingest/packs/html-to-text.test.ts`
  (the dnd5e pair in the real-fixture pins, the nested-bracket one nowhere yet),
  so the day one is fixed the pin is the thing that changes.
- **A top-level ARRAY JSON file is ONE document, so the failure REASON lies about
  what happened** (docs/17 row 147, recorded not fixed). All five JSON lanes
  share `packs/text.parseJsonDocs`, which tries `JSON.parse(trimmed)` first — so
  a file holding `[{…creature…}, {…creature…}]` yields ONE document, the array
  itself, which every adapter skips as "not a document I know". The import then
  fails with the zero-valid reason, and the reason is false comfort rather than
  a lie of omission:
  ...
  array form yields `entries: 0, skipped: 1, failures: 0` and the message above.
  **This is NOT duplication** — all five copies always agreed on it — it is a
  hole in the failure REASON: the count is right and the noun is right, but
  nothing says "this file's documents are wrapped in an array". Fixing it is a
  behaviour change (does an array file become N documents, or a named failure
  saying so?) with an import-report consequence, so it is out of scope for the
  centralization landing. **Untested today:** exactly ONE test feeds an array
