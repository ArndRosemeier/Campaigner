...
- **STILL OPEN (docs/17 row 257) — family E's BATTLE-ROW writer is not repointed, and one render
  divergence is deliberately NOT absorbed.** The adoption seam (`db/libraryAdopt`, §2.1) copies a
  referenced GLOBAL LIBRARY artifact into its campaign and repoints the two DECLARED holder shapes
  (a roster `npc-ref` target, an artifact's `links[].targetId`) at v26 upgrade time, on the
  start-up retry, and at the editor's autosave for a reference that is ABOUT TO BE BORN. NOT yet a
  member of that declared set: `battle.board.tokens[].artifactId` and the stage snapshot
  (`domain/battle`) — a battle seeded from a library npc keeps a pointer to the LIBRARY row. It is
  deferred rather than quietly left because the degradation is SILENT
  (`db/creatureRepo.tokenCreature` resolves through the any-scope getter, so the card still works):
  a token whose library row is later deleted keeps showing nothing, with no named reason.
  ALSO NOT this seam's business: the E1 render divergence for a reference it CANNOT reach — a gone
  library row, or an `import-anyway` landing — where `db/monsterResolve` passes the campaign-only
  `getArtifact` while every other reader passes `getAnyArtifact` (`db/battleSeed`,
  `db/mob-portrait-queue`, `db/creatureRepo`); that is board row 258 and needs its own decision
  (one getter for the reference, or a declared campaign-only contract), not a silent absorption
  here. The `links[]`-not-remapped-on-import gap is the SAME class and is board row 256.
