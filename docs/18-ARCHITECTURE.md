...
framing is held SEPARATELY (`persona-panel`'s `refillFraming`, keyed to the target
artifact) and the two meet in the ONE composer
(`llm/additionalInstruction.withAdditionalInstruction`) at `start()`, so the words
ride the `Additional instruction:` paragraph `directInstructionFor` reads. An
EMPTY box leaves the framing BYTE-IDENTICAL, so the deliberate no-instruction arms
are untouched.
