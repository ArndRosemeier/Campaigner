...
| **Convert a mob's library citation into an authored COPY (the one-representation migration)** (docs/17 row 248) | `db/mobCopyRepair.repairMobCopies({ tx, reason })` — THE one seam, taking the Dexie TRANSACTION and never the `db` singleton (the v20 `creatureRepair` precedent: an upgrade body runs before the upgraded instance is usable). Dexie **v24** calls it with `reason: 'upgrade'`; `db/mobCopyRetry.retryMobCopies()` calls it in an ordinary transaction from AppShell's start-up effect when `settings.mobCopyRepair.unconverted` is non-empty (the owner-decided failure arm). ROSTER arm: the ONE copy seam `domain/libraryCopy.copyCreatureStats` (docs/17 row 255a) over TX-BACKED lookups, writing `{type:'inline', statBlock}`, `sourceLine` and `originToken`. NPC arm: `data.creatureRef` → `statBlock` + `sourceLine` + `originToken` (docs/17 row 255b added the token: it is the row's portrait AND reuse identity once the pointer is gone), the ref DROPPED. A row that cannot resolve KEEPS its pointer and is reported BY NAME in `settings.mobCopyRepair`; `domain/mobCopyRepair.formatMobCopyRepair` is the ONE sentence and AppShell says it once (`notified`). IDEMPOTENT: a second run finds nothing and reports all-zero. Its pointer READS go through the legacy seam above (`storedRulebookCitation` / `storedNpcCitation` / `withoutLegacyNpcPointer`), so this module spells no pointer field itself | a second copy mechanism at a render/read site; a `db`-bound wrapper called inside the upgrade tx; a placeholder or empty stat block for a row that did not resolve (the pointer survives instead); dropping the pointer on a row that could not be converted (it is the retry's only handle); **a `catch` around the whole pass instead of the per-row `guard`** (it would lose the row identity and the error text); **a second name-matching implementation** — `domain/libraryCreature` is the ONE (the row below) |
| **Copy a library creature's stats onto a module row AT WRITE TIME — the ONE copy operation, shared by the migration's backfill and every live mint** (docs/17 rows 255a/255b) | `domain/libraryCopy.copyCreatureStats(citation, fallbackName, lookups)` is THE one copy: PURE, tx-callable, injected lookups; it returns the library `statBlock`, the STAMPED `sourceLine` (`creatureOriginLabel`, which used to be composed at read time) and the opaque `originToken` (`libraryCreatureKey`), or a NAMED unresolved reason. `db/libraryCopy.copyCreatureStatsFromDb` is the ONE live wrapper (over `db/creatureRepo.creatureLookups`, so a copy and a resolution cannot disagree about the content-hash fallback). The Dexie v24 backfill (`db/mobCopyRepair`) calls the SAME seam with its tx tables, and the live WRITE paths call the wrapper: `llm/runEngine.rosterMobCopyFor` (encounter finalize), `features/campaign/components/monster-source` (the editor picker), `features/play/battle/spawn-picker-logic.buildMobPickEntry` (battle spawn). `db/creatureRepo.castCreatureAsNpc` (the CAST path, docs/17 row 255b) calls the PURE seam directly with `creatureLookups()` — that module IS where the wrapper's lookups come from, so importing the wrapper back would be a cycle — and is the fourth declared caller. A vanished chunk is a LOUD refusal (`domain/libraryCopy.creatureCopyRefusal`) — a write path never mints a pointer as a consolation. `tests/architecture/one-library-copy.test.ts` is the exactly-one pin (the token is minted only by the identity layer + the seam; the copy operation is DEFINED once; no write path composes an origin label) and the holder list is updated whenever a path joins; `tests/features/mob-copy-on-write.test.ts` is the DIFFERENTIAL pin (the pure seam, the generator and the picker produce ONE identical copy) and proves the backfill equals a fresh write-path copy | a SECOND copy mechanism at a write site (the fragmentation this arc exists to remove); composing `creatureOriginLabel` or `contentIdentityFor` at a write path (the stamped line comes off the seam); minting a `rulebook`/uuid-only source when the chunk cannot be copied (refuse loudly instead); calling a `db`-bound wrapper inside the upgrade tx (or reaching back into `db/libraryCopy` from the module it imports) |
| **Adopt a GLOBAL LIBRARY artifact into a campaign as a REAL COPY — the library row SURVIVES** (docs/17 rows 257/259) | `domain/libraryAdopt` is the PURE half: `adoptedArtifactRow(source, campaignId, images)` (fresh id, campaign scope, `moduleId: null`, `currentRevision: 1`, the `copiedFromArtifactId` origin stamp, cloned role-preserving image ids) and `repointLibraryReferences(holder, resolve)` — the ONE artifact-row rewriter of its DECLARED holder shapes (`links[].targetId`, and the roster `npc-ref` target through `domain/rosterRefs.repointRosterArtifactIds`) — whose stored-row arm is `repointArtifactRow` (a fresh revision). The THIRD declared shape is the BATTLE holder (docs/17 row 259, the LAST runtime dependency on the library): `battleLibraryReferenceIds(battle)` collects `board.tokens[].artifactId` AND the `board.stage.tokens` snapshot, `repointBattleRow(battle, resolve)` is the rewriter at the `db.battles` address (in the same transaction as the copy; it also remaps a DERIVED `npc-ref` seed row's id, which is stored under the artifact id), and `danglingBattleTokens(battle, knownIds)` is the loud arm — a token that is neither a known artifact (any scope) nor one of the battle's frozen seed handles is NAMED in `report.unresolved` instead of rendering nothing, because deleting a SHARED library row scrubs no campaign's tokens (`db/artifactRepo.deleteArtifact` scrubs only an OWNED artifact's). The battle's SEEDING ENCOUNTER is the RECORDED DELIBERATE EXCEPTION (docs/17 row 263): `battle.encounterArtifactId` is the battle's IDENTITY key (`battleRepo.getBattleByEncounter`), so it is NEVER collected and NEVER repointed — re-pointing it would split the board — and `danglingBattleEncounter(battle, knownIds)` NAMES a gone row in the same `report.unresolved` (the exception's LOUD half; **v28** runs the same seam so the arm reaches an install where v27 already finished). `libraryReferenceIds(holder)` is the ONE artifact collector, shared by the migration and the write path. `db/libraryAdopt.adoptLibraryArtifacts({ tx, reason, pendingRefs? })` is the ONE transaction-taking seam and imports no `db` singleton, so the Dexie **v26** upgrade body (the artifact holders) and **v27** (the battle holders, added because a landed upgrade body never re-runs) and **v28** (the seeding-encounter arm — the same reason, one arm over) , `db/libraryAdoptRetry.retryLibraryAdoptions()` and `db/libraryAdoptLive.adoptDraftLibraryReferences(campaignId, holder)` (the editor autosave's write-time half) all call it; the COPY and the REPOINT land in the SAME transaction. `db/artifactRepo.duplicateArtifact` DROPS the origin stamp, and `domain/artifactRevision.artifactRevisionRow` is the ONE revision row the artifact transports write. The roster shape lives in `domain/rosterRefs` (`rosterArtifactIds` is the ONE detector, re-exported by `db/artifactAutoPromote`). Pinned by `tests/db/libraryAdopt.test.ts` (20 pins: idempotence on the stored origin, every declared holder shape, the battle board AND stage repoint, the dangling-token NAMING arm, the retry's second run reporting all-zero, cloned images with their role, the gone-row arm, prose resolution, one copy per campaign, the write-time arms), the real-render acceptance pin in `tests/features/battle-token-portrait.test.tsx` (a token seeded from a LIBRARY npc renders with the library row DELETED), plus the exactly-one source scan in `tests/architecture/one-library-copy.test.ts` | a SECOND copy mechanism (per-shape or per-site); **extending `moveScope`/`adoptIntoCampaign`** — that verb MOVES the row and empties the SHARED library, which is exactly what the owner's answer forbids; SHARING an image id instead of cloning it (an import re-scopes the library's blob); `reanchorImages` (it moves the library's blobs); deciding idempotence from a NAME or from bytes; repointing a target that has no copy (leave the reference intact — a gone library row keeps its loud missing arm, never a placeholder); writing the repoint without its copy, or in a second transaction; a `db`-bound wrapper inside the upgrade tx; **leaving a battle token out of the declared set** (docs/17 row 259 — it degrades SILENTLY, since `db/creatureRepo.tokenCreature` resolves through the any-scope getter); repointing only `board.tokens` and not the `board.stage.tokens` snapshot, or the token without its DERIVED seed row's id (the frozen stats would be orphaned); reporting a battle token that points at a frozen seed handle (a synthetic id is a legitimate answer, not a dangling reference); **re-pointing `battle.encounterArtifactId`** — it is the battle's identity key and the RECORDED deliberate exception (docs/17 row 263): a gone row is NAMED through `danglingBattleEncounter`, never repointed, or the board splits |
...
| **Show a cited npc's BORROWED numbers** (the Aunt Agatha row's stats; docs/17 row 134, docs/11 §A cited row's REFILL; **a MIGRATED copy is shown from its own stored block + stamp — docs/17 row 248**) | `domain/encounterResolve.resolveDerivedNpcStats(npcName, citation, lookups)` — THE one derived-stats rule: an empty citation THROWS (`creatureRefIsEmpty`), a citation the library cannot supply answers the named `missing ref (<the citation's own creature name>)`, and a resolved one answers the library block under `derivedStatOrigin` (`NPC: <name> (stats from <book p.N>)`). The encounter roster's `npc-ref` arm calls it and `db/creatureRepo.resolveDerivedNpcStats` is the repo-wired wrapper top code reads. **A CONVERTED row (Dexie v24) carries `statBlock` + `sourceLine` and no `creatureRef`: `resolveMonsterEntry`'s `npc-ref` arm discloses it through the SAME `derivedStatOrigin` composer over the STORED line, so the read no longer touches the library at all.** RENDERING is ONE component, `features/campaign/components/borrowed-stats.BorrowedStatBlock`, MOUNTED by `features/campaign/components/kind-forms.NpcForm` (the artifact editor's details) and by `features/play/artifact-cards.NpcCard` (the read-only card the module reader's peek modal and session mode show) — it draws the block READ-ONLY with a `Borrowed from the library` badge plus the origin label, renders a named destructive notice for a missing creature, and raises a thrown read through `lib/toast` | a second derivation or a hand-rolled stat extraction in a component; a `StatBlockCard` rendered off `data.creatureRef` at a surface of its own; writing a block onto a cited row (the pair is refused by name); offering a cited row the `Add stat block` button (the cited-row refill refuses it before any model call); rendering a blank/greyish stat area for a missing library creature (defaults that mask a failure are forbidden); **a SECOND composer for the copied-row disclosure** (the migrated arm calls the ONE `derivedStatOrigin`; the count in `tests/features/cast-row-borrowed-stats-scan.test.ts` moved 2→3 for that second CALLER, and the single-holder assertion is the guard against a second implementation) |
| Backup / export / import | `lib/backup.ts` — **the whole-DB SAVE is ASYNC and CHUNKED (docs/17 row 265)**: `buildBackup({ onProgress })` reads ONE table at a time, serializes rows in bounded batches and pushes the manifest JSON and each image binary into fflate's STREAMING zip in 1 MiB slices, yielding to a MACROTASK (`yieldToEventLoop`) between them; output is still ONE zip with the same entries and manifest shape, images keep compression level 6, and the report is per table/per image. The interrupted-run marker (`markBackupInFlight`/`noteBackupSettled`/`backupRunWasInterrupted`, a localStorage key) is the aborted-tab arm. Restore rides `db.transaction('rw', db.tables)` + the chunk door and is still a synchronous `unzipSync` (see §5); `OPTIONAL_TABLES` lists `moduleVersions` beside `pdfFiles`/`mobPortraits`, so a PRE-undo zip — one whose manifest predates the v19 table — restores with an EMPTY undo stack instead of failing the missing-table check) / `lib/exportImport.ts` (campaign export v2: modules/battles/runs + `dependencies` manifest, and `RETIRED_EXPORT_TABLES`/`retiredTableRows` counting the rows of a table this build no longer has — reported on import, never dropped in silence; import = ONE tx over the eight tables, array form, with module/artifact re-id maps rewriting every reference — an artifact whose exported `moduleId` is not in the file demotes to campaign level ONLY on a v1 file (the documented legacy rescue) and THROWS loudly on v2, the battle path's precedent, because demoting would move a row out of its module; `parseExportTolerant` is the import boundary's tolerance stage — strict first, then per-row retired/drift triage with skip-count, reassembled output strict-parsed before return) | table-by-table writes that can strand a half-import; a second whole-DB zip builder beside `buildBackup`, or a per-table `zipSync` reintroduced "for speed" (it blocks the main thread on the one path a GM runs with a session waiting) |
...
| **Wrap a pack adapter's synchronous parse in the promise-based adapter contract** — THE one pack-adapter promise seam (docs/17 row 214) | `packs/types.asPackFileParser(parseFileSync)` in `src/ingest/packs/types.ts`, beside the `PackAdapter` contract whose `parseFile(fileName, bytes): Promise<PackFileParse>` it enforces (and beside `fileToPackInput`, this module's other runtime adapter helper). Each adapter KEEPS its own synchronous `parseFileSync` — the parsing is per-lane and NEVER shared — and states the contract in ONE line: `const parseFile = asPackFileParser(parseFileSync);`. It lives here and NOT in `text.ts` because `text.ts` is the ingest layer's DOCUMENT-conventions home (HTML→text, the document stream) while this is the adapter CONTRACT. A non-`Error` throw is re-wrapped as an `Error` rejection because `packImport` reads the rejection's `.message` (AGENTS rule 1). Seven byte-identical copies existed until the duplicate-body tripwire named them as group `4849c733c9136aa8`; the fold deleted that baseline line | the NOT-Z column: **a per-adapter wrapper** — no `Promise.resolve(parseFileSync`, no `instanceof Error ? error : new Error(String(error))` and no `function parseFile(` outside `types.ts`, red by the SOURCE SCAN in `tests/architecture/one-pack-file-parser.test.ts` (which also requires all seven adapters to import the seam and call it exactly once, and no eighth file to reach for it); a second wrapper or helper for the same contract anywhere under `src/ingest/packs/` |
| Ground a mob portrait in a creature chunk | `portraitGroundingForChunk` (`llm/imagePromptDraft.ts` — stat-exempt: size + creatureType identity plus traits/actions/reactions/legendary prose, every numeric field out by field, cap at `IMAGE_PROMPT_GROUNDING_MAX_CHARS` (10,000 since docs/17 row 223 — the owner raised it from the never-authorized 800); null statBlock falls back to raw `chunk.text` as the loud residual render risk) + the default-on text-render guard (docs/11 D5, generalized: `IMAGE_TEXT_NEGATIVE` is the DEFAULT `negative` of the shared Illustrator contract — covers, entity images, portraits, the run-engine prompt draft, classic stylize, and the appearance shortcut are all guarded; the `negative` option stays the explicit-override seam; `MOB_PORTRAIT_TEXT_NEGATIVE` survives as the identical alias; AMENDED by docs/17 row 224 — the guard no longer forbids text wholesale: the bare terms `text`/`letters`/`numbers`/`words`/`label` are dropped and the ONE exported `IMAGE_TEXT_SPARING_CLAUSE` ("Unless requested otherwise, use text sparingly.") rides the composed prompt of both `buildImagePrompt` branches and the classic-stylize template, so a requested treasure map / legend / confession letter is never overridden by the Avoid list; the vision dungeon path is the one documented carve-out — its tailored plaque clause instead of the list, and the sparing clause is deliberately NOT added there because that plaque clause is load-bearing for the locate pass) — docs/11 D5 | feeding raw `chunk.text` into `buildImagePrompt` (image models render stat digits into portraits); a bare text/letters/numbers/words/label Avoid item (an image model reads it as "no text at all" — the row-224 defect); a second spelling of the text budget (`IMAGE_TEXT_SPARING_CLAUSE` is the ONE constant) |
...
| **Reconcile a PDF-IMPORT row a discarded page left at `status: 'processing'`** (docs/17 row 266) | `ingest/ingestReconcile.reconcileInterruptedPdfImports()` — called from `AppShell`'s mount effect ONLY (app START: a discarded tab RELOADS, and this page has started no import when the shell mounts). It reads the population through `db/rulebookRepo.listProcessingPdfBooks()` (the indexed `status` column, then `origin === 'pdf'` AFTER the legacy-row parse) and fails each row through `db/rulebookRepo.failInterruptedPdfImport(id, INTERRUPTED_PDF_IMPORT_MESSAGE)`, which re-reads BOTH `status` and `origin` INSIDE its `rw` transaction — so it is idempotent and a pipeline that finished between the read and the write is never overwritten. `'error'` is EXACTLY the status the Rules page shows its `Retry…` menu item on, so the row gains the way forward it never had; the batch toasts ONCE with the count and the control named. Ownership: `lib/generationLocks.isGenerationLockHeld(ingestLockName(bookId))` — the ONE cross-tab lease, held by `ingest/ingestFiles.ingestPdf` across the extraction AND the persistence (docs/17 row 266), so a start-up in another tab leaves a live import alone. Deliberately NOT on `onPageResumed` (the module twin IS): a merely suspended tab resumes its OWN extraction, so failing that row on the way back would invent the defect this seam removes — the rule that keeps RUN rows out of the visibility path | treating a persisted `'processing'` as PROOF somebody is writing; a silent reset to `'ready'`/`'draft'`; failing a PACK row (it also carries `'processing'` but has no file to re-select — §5 names its owed recovery); reconciling at mount WITHOUT the in-transaction status re-read (a just-finished import would be overwritten); wiring it to `onPageResumed`; a second recoverable-status rule or a second reconcile path (ONE constant, ONE transaction, ONE batch call) |
...
- **OWED, NAMED BY docs/17 row 266 (the PDF-import reconcile slice) — PACK books are
  ...
  rows ONLY, deliberately: a pack book also carries `status: 'processing'` but has NO
  file to re-select, so the PDF recovery sentence would be a lie on it. A pack book
  left `'processing'` by a discarded tab — or by `importPack` throwing after
  `createPackBook` — therefore still has no recovery affordance of its own; that slice
  is owed (board QUEUE row 241: `failBook` exactly once + a pack reconciler + the
  Spells page naming a non-ready same-system book). (b) `features/rules/RulesPage`'s
  ...
  introduced here: this slice only guarantees the wedged row reaches a status on which
  ...
  `bookId` keying). (c) **jsdom cannot measure memory pressure or a tab suspension.**
  The "file is held ONCE" property is pinned as buffer IDENTITY (`ingestBuffers`, plus
  the worker path's `putBookPdf` bytes being the very `ArrayBuffer` the worker was
  handed) and the reconcile on ROW STATE and the lease guard; the device test — a real
  iPad tab discard mid-extraction — is the owner's. The pre-266 double-hold was
  `arrayBuffer.slice(0)`; a revert reds the identity pins.
...
- **CLOSED (docs/17 row 259) — family E's BATTLE-ROW writer IS repointed: the last runtime
  ...
  re-runs, so editing v26's body could not have reached him), the start-up retry heals, and the copy
  ...
  `settings.libraryAdopt.unresolved` — the SILENT arm this row closes — and the acceptance is proved
  through the real render path with the library row DELETED
  (`tests/features/battle-token-portrait.test.tsx`).
  **CLOSED (docs/17 row 263) — the seeding encounter, the getter divergence and the scrub gap.**
  (a) The battle's SEEDING ENCOUNTER is now the RECORDED **DELIBERATE EXCEPTION**:
  `battle.encounterArtifactId` is a reference to the seeding encounter artifact, and a LIBRARY-scoped
  encounter IS runnable (`features/campaign/components/artifact-editor.tsx`'s `EncounterRunAction`
  opens the module picker for `moduleId === null`, then `run-battle-seed`) while `BattleSurface`
  re-reads it with `getAnyArtifact` — so a battle seeded from a global encounter keeps a
  provenance/spawn-source dependency on the library row. It is KEPT because the battle is keyed BY
  that id (`getBattleByEncounter`) and re-pointing it would change the battle's identity (a second
  board for the same encounter). The exception's LOUD half is
  `domain/libraryAdopt.danglingBattleEncounter`, which NAMES a gone row in
  `settings.libraryAdopt.unresolved` beside the dangling tokens; Dexie **v28** runs the SAME seam so
  that arm reaches an install where v27 already finished (a landed upgrade body never re-runs).
  **DO NOT "FIX" THIS BY RE-POINTING THE ID.** (b) CLOSED — the E1 render divergence for a reference
  ...
  still the loud `missing ref` arm (nothing was softened). The `links[]`-not-remapped-on-import gap
  is the SAME class and is board row 256, still open. (c) CLOSED —
...
- **CLOSED (docs/17 row 255b) — `monsterEntry.originToken` IS INERT again: a copied mob's battle
  card no longer resolves through the library.** The finding (a read-only probe at base `14e63fb`)
  was that `rosterEntryCreatureIdentity` reads the token FIRST and feeds `token.creatureKey` at
  seed, while `tokenCreature` mapped that `chunk:` key to a LIVE chunk read — so a mob whose row
  WAS migrated still lost its battle card when the pack was uninstalled. The seed row now FREEZES
  the copy (`seedFighter.statBlock` + `.originLabel`) and `tokenCreature` prefers it
  (`BattleSurface` passes the row), so the token is an IDENTITY, not a resolver — pinned through
  the real render path with the chunk DELETED
  (`tests/features/battle-token-portrait.test.tsx`) plus the differential arm in
...
- **CLOSED (docs/17 row 248's remaining slice): a per-row throw inside the v24 mob-copy
  ...
  entry (`unexpected: true`, carrying the error's own `message`) and the run COMPLETES.
  The entry is rendered in its own register by `domain/mobCopyRepair.formatMobCopyRepair`
  ("this is a defect, not a missing pack, so please report it"), so a caught CODE defect is
  never swallowed (AGENTS rule 1) and the row keeps its pointer for the retry either way.
...
- **STILL OPEN — the pointer ARMS are not deleted, the module arm is not converted, and
  TWO of the write-time mints remain (docs/17 rows 248c/255a, measured at HEAD).** THE
  WRITE-TIME HALF IS PARTLY LANDED (docs/17 row 255a): `domain/libraryCopy` is now THE one
  copy operation, and the Dexie v24 backfill plus the three ROSTER write paths
  (`llm/runEngine.rosterMobCopyFor`, the editor picker, the battle spawn picker) all call
  it, so no NEW roster `rulebook` pointer is born and a vanished chunk is a loud refusal.
  **BOTH write/read blockers have LANDED (docs/17 row 255b), and family C has LANDED TOO
  (docs/17 row 255c) — the three blockers this entry named are all CLOSED:**
  (i) **CLOSED** — `db/creatureRepo.castCreatureAsNpc`, the module generator's ONLY cast path,
  now COPIES through the ONE seam and writes `statBlock` + `sourceLine` + `originToken`, so every
  cast NPC (and therefore every module `entityKinds[].bestiary` FULFILMENT, which is that cast
  row) owns its numbers and mints no pointer; the reuse identity that survives the dropped ref is
  the copy's identity key (`domain/creature.npcCreatureIdentity` over the stored `originToken`,
  with a legacy `creatureRef` answered by the SAME rule), and the ~10 cast-path test files were
  rewritten to the new truth. (ii) **CLOSED** — the BATTLE CARD read chain (item 4 below): the
  seed row FREEZES the block and `tokenCreature` reads THAT, proven through the real render path
  with the chunk DELETED. (iii) **CLOSED by docs/17 row 255c** — family C SPELLS: the ONE copy
  operation now stamps each bare assignment with the library's FULL entry (its
  `publication {title, license}` line included), `domain/mobSpells.mobSpellChips` reads the
  copied entry FIRST so a copied mob renders with the library absent, and the migration's
  tx-backed arm does the same through the transaction's tables. **THE EXPORT-SIZE DELTA IS
  MEASURED, not estimated** — the method and the numbers are in the row-255c ledger entry
  (payloads parsed from the four real Foundry spell fixtures through `ingest/packs/pf2e-rules`;
  the normalized `spellData` is ~28% of the raw fixture, and the delta scales per ASSIGNED
  SPELL, i.e. once per copied caster's spell list, duplicated on every copied row). The
  ...
  2. **Converting `entityKinds[].bestiary` to a copy is a FORK, not a code change.** The
     ...
     "convert it to a copy" means either writing the library's block onto a module entity
     record (a SECOND stored stat representation beside the cast row — the fragmentation
     this arc removes) or creating a cast artifact for an entity that may never be generated
     (duplicating `castCreatureAsNpc`, the ONE cast seam). `libraryCitationForSlot` makes
  ...
  4. **CLOSED by docs/17 row 255b — THE BATTLE-TOKEN CHAIN NO LONGER RESOLVES THE `chunk:`
     TOKEN.** It was the one place the "identity, not a resolver" statement was FALSE:
     `db/creatureRepo.tokenCreature` mapped a `chunk:` key straight to a LIVE
     `resolveCreatureCitation` read, and `BattleSurface` printed that as the token card's stat
     block, so uninstalling a pack stripped an already-seeded battle of its AC and attacks (the
     token's synthetic `artifactId` has no artifact row behind it, so the
     `?? selectedArtifact.data.statBlock` fallback could not save it). `seedFighterSchema` now
     carries the frozen `statBlock`/`originLabel`, `db/battleSeed` stamps them, `tokenCreature`
     prefers the battle's frozen row over any library read, and `BattleSurface` passes it — the
     token is an identity again, and the fix is pinned with the chunk DELETED through the real
     render path.
...
- **The duplicate-body tripwire's FIRST capture found 16 duplicate groups /
  ...
  **The union pin closes the gap the two scoped inventories cannot (docs/17
  ...
  - **`parseFile` ×7 is FOLDED (docs/17 row 214)** — the seven adapters now
    ...
    item is history, not debt. `titleCase` ×3 and `publicationSourceLine` ×2
    remain — the same pack-adapter duplication family as `isRecord`; their fold
    is still owed (separate slices). The `publicationSourceLine` pair is now the
    `src/` half of row 215's measured cross-tree class; the TEST-side reference
    copy was the one folded.
...
- **`levelDistance`/`levelDistanceTo` ×2 is still owed a fold** — recorded
