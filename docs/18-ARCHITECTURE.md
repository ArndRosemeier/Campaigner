...
- **THE MOB-COPY ARC'S REMAINING SLICE (docs/17 row 248, landed as the v24
  ...
  2. **`entityKinds[].bestiary` (the MODULE arm) is NOT converted.** It is a
     cast REQUEST resolved by NAME at finalize, and the recipe's LOUD UNKNOWN (b)
     is CONFIRMED by measurement: `features/modules/entity-batch.libraryCitationForEntity`
     reaches `db/creatureRepo.listLibraryCreatures` → `db.rulebooks`, which is not
     callable inside the upgrade transaction. Converting it needs an in-tx path
     through the ONE library name-matching seam — reusing `listLibraryCreatures`'s
     comparison (or folding it there), NEVER a second copy of the name match
     (AGENTS §Centralization obligation 4). The measured blast radius at
     `c3850c3`: `'rulebook'` 40 hits/22 files, `'npc-ref'` 35/21, `creatureRef`
     75 refs, 20 `src/` files and ~10 test files touch the union.
