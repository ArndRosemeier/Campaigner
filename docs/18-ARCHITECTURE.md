...
| Regenerate a mob / invented-creature portrait | `features/campaign/mob-portrait-queue.regenerateMobPortraits` (rulebook: validate → republish canonical slots fresh → `removeImageFromArtifact` detach → normal enqueue) / `regenerateInventedCreaturePortraits` (uncited: materialize → detach → normal chunk-less enqueue) — detach-then-enqueue is the one way; between detach and the new cover, tokens show initials (accepted) | a second detach/enqueue path; re-enqueueing an imaged artifact expecting fresh bytes (the skip branch + cache read-through return the OLD art — a no-op regen); detaching without re-enqueueing (strands initials) |
