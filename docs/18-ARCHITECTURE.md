...
| **Record a recently used GLOBAL chat model** (docs/17 row 193) | `db/settingsRepo.recordRecentChatModel(model)` — THE one recording seam: ONE `rw` transaction reads the settings row, merges through the ONE pure `domain/settings.withRecentChatModel` (trim, move-to-front, dedupe, cap `RECENT_CHAT_MODELS_CAP` = 8, oldest dropped) and writes through the ONE `updateSettings` (validation + the prompt-styles carry-forward inherited, never re-implemented). Callers: `features/settings/model-picker`'s `choose` (the picker changes the global model) and `llm/runEngine.recordChatModelInUse` at `executeFrom`'s top — a run whose chat model resolves to the GLOBAL default; an image-mode run and a persona-override run are deliberately skipped, because the global model is not in play there. The list lives on the ONE settings row as `recentChatModels: z.array(z.string()).default([])` (additive; no migration, no Dexie version) | a component-side read-modify-write (two concurrent recorders read a stale list and lose an entry — AGENTS rule 1); a second ordering rule; recording a persona override, `fallbackChatModel` or an image model; a second settings write path |
...
| **Change the GLOBAL first-try chat model from anywhere, and offer what was recently used** (docs/17 row 193, docs/05 §Top bar) | `features/settings/model-picker.ModelPicker` — mounted in `app/layout/TopBar` inside the primary nav IMMEDIATELY after the Settings entry (every route), it edits the SAME `settings.defaultChatModel` the Settings page edits and reads `settings.recentChatModels`; the "Recently used" group is rendered `shouldFilter={false}` in stored order (filtering only REMOVES rows), free-form entry stays available because the setting is free text, and its account list comes from the ONE option seam below. Recording rides `db/settingsRepo.recordRecentChatModel` (above); the picker's own write is `updateSettings({defaultChatModel})` then that seam | a second stored model or a second picker-only setting; a component-side recents read-modify-write; a picker that re-sorts recents (alphabetical/score order — the owner asked for recency); a closed list that drops free-form entry; an empty Account-models group that reads as "no models" when the real cause is a missing key or a failed fetch (rule 1) |
| **Offer the account's model ids to a model-choice control** (docs/17 row 193) | `features/settings/model-options.listModelIds()` — THE one account model-id option source (`(await listModels()).map(m => m.id)`, the transport cache beneath it), used by `ModelInput`'s DEFAULT browse list (chat/embedding/persona fields) and by the top-bar `ModelPicker`; `fetchOptions` remains the escape hatch for a DIFFERENT list (image models, `listImageModels`). `tests/architecture/one-model-option-source.test.ts` pins the exact `listModels(` population (the transport definition, this seam, and the two named different-purpose consumers: the Settings "Test key" probe and `ReasoningEffortSelect`'s full-row reasoning metadata) | a copied `(await listModels()).map(...)` at a new model-choice surface (the picker's would-be second `/models` fetch); folding the Test-key probe or the reasoning-metadata reader into an ids-only seam (they need the live endpoint / the full rows) |
