...
- `15-GRAPH-RETRIEVAL.md` — graph-aware retrieval (spec-first, decisions open): the retrieve step detects entities in the run brief via wiki-link tokens/names/aliases against the reader pool, expands them through the derived wiki graph (co-mention), and persists a bounded campaign-grounding section with the stored retrieve output — recommendation: co-mention only, encounter general grounding opts in, no new searches/embeddings; statblock citable pool, roster, quickfind and the relations editor untouched
