...
- `11-ENCOUNTER-GENERATOR.md` — generated encounters: LLM brief → deterministic grid layout → schematic → stylized battlemap (img2img) → human pick (no verify step — D14: the user is the judge; Regenerate candidates re-runs stylize only); room-veiled mobs seeded onto the board; unattended module-generation path; Dungeon preset (D10, owner-ratified): generation-time ×2 grid tier + multi-room complex bias, persisted as `preset`
- `12-BESTIARY-PACKS.md` — bestiary pack imports: per-source adapters turn user-imported machine-readable creature data (Foundry pf2e/dnd5e-SRD packs) into exact `statBlock` chunks in the existing pipeline; pack roster grounds encounter monster selection; never bundled or shipped
- `16-BESTIARY-FETCH.md` — user-triggered bestiary-pack fetching from the pinned upstream repos (pf2e `v14-dev`, dnd5e `6.0.x`): Settings-menu card, curated pack recipes + advanced full-repo listing, raw.githubusercontent per-file fetch feeding the unchanged `importPack` (doc 12 §1/§2 amendment); adapters stay network-free, nothing bundled
