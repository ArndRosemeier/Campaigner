...
- `11-ENCOUNTER-GENERATOR.md` — generated encounters: LLM brief → deterministic grid layout → schematic → stylized battlemap (img2img) → vision verify; room-veiled mobs seeded onto the board; unattended module-generation path; Dungeon preset (D10, owner-ratified): generation-time ×2 grid tier + multi-room complex bias, persisted as `preset`
