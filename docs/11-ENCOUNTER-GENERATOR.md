...
| D12 | **Asymmetric per-room budget loop (owner-specified)**: each complex room carries `targetLevel` (additive, optional; defaults to the encounter's parsed levelHint) and the assigned creatures' levels are summed against a documented band — **too easy ⇒ ship silently (owner: fine)**; **too hard ⇒ lower that room's targetLevel a step (floor 1) and retry through the encounter brief's EXISTING single repair turn** (budget issues join the issue list like coverage/source issues); **after the bounded retry still over ⇒ LOUD advisory** persisted on the step output AND `data.budgetAdvisory` on the artifact — never silent, never a failed run. The final (possibly lowered) targetLevel persists on the room, visible and owner-editable. dnd5e band = our own documented approximation (verbatim rationale below, mirroring the treasure-ladder licensing stance, docs/12 §13.2/§14); pf2e ships NO numbers — GM Core verbatim from retrieved excerpts when present, else the always-on loud advisory. The in-place Smith content fill runs the same loop over a RECONCILED partition (see below). **Amended (fill-grade arc, owner-ratified): a COMPLEX inverts the asymmetry — every room stocks a real fight against a drawn `fillGrade` expectation ('empty' repairable, 'under' loud); the "no lower bound" call now holds for SINGLE arenas only, the roster is sized at the real seam (prompt + bounded map-queue expansion — SHAPE-gated, see the amendment), and the in-place fill packs by nearest-band fit. See "D12 amendment — fillGrade stocking" below.** **Amended (docs/17 row 180, owner request): the per-system rule is now a SELECTABLE, PERSISTED policy (`encounterBudgetPolicy`: `'system'` = the compatibility mapping above, `'pf2e-budget'` = the pf2e-native numeric path, `'verbatim'` = no numbers), resolved ONCE per run and stamped on the module row at creation — a `pathfinder2e` module DEFAULTS to `'pf2e-budget'` so a filled dungeon actually challenges the party, a module row without the field (every legacy row) still reads `'system'` byte-identically, and `'verbatim'` keeps the no-numbers licensing stance selectable. See "D12 amendment — the selectable budget policy" below.** **Amended (docs/17 row 190, owner request): a module also carries a five-step `difficulty` (`much-easier`…`much-harder`, middle = `normal`) — a SIBLING of the policy that scales the ONE standard-encounter number both numeric modes use (the dnd5e `T + 2` band and Campaigner's own pf2e approximation), so the check, the fill-grade expectations, the stocking cap and the brief's numbers move together; a legacy row reads `normal` (today's numbers, byte-identical) and under `'verbatim'` no number is computed, so difficulty is a prompt DIRECTION only and the no-numbers stance is untouched. See "D12 amendment — the module difficulty setting" below.** |
...
- **The single-room level/difficulty INPUT (docs/17 row 228, owner report).**
  A TARGETED Smith content fill (Repopulate / Regenerate everything's content
  leg / a panel refill) now carries the party level line and the
  module-difficulty/band clause in its draft prompt, rendered by
  `runEngine.encounterInputGuidanceFor` through the SAME seams the Cartographer
  brief uses — `encounterPartyLevel` + `partyLevelLine` (ONE level sentence)
  and `roomBudgetGuidanceFor` (which embeds the ONE
  `moduleDifficultyGuidanceFor` clause plus the band it refers to). Fresh
  creates (no target) and the prose-only pass render neither, so their prompts
  stay byte-identical. The in-place fill keeps the target's stored
  `data.levelHint`/`data.difficulty` when it states them (the reply may not
  rewrite what the owner retuned), stamps target-less rooms from that KEPT
  level so the deterministic budget check reads the encounter's own level, and
  speaks any disagreement between the reply and the kept value LOUDLY on the
  persisted `budgetAdvisory` + step notice. The warning stays a warning: D14's
  "the user is the judge" rule is untouched and no refusal was added.
...
at run start). **Corrected measured fact (docs/17 row 228):** at row 195 that
fresh read reached the PROMPT only on the COMPLEX (Cartographer) branch — the
single-room branch's generic Smith brief stated no difficulty at all and
`difficulty` reached the finalize CHECK only, so "the very next Repopulate is
built at the changed difficulty" was true of the Cartographer branch alone.
Since row 228 the single-room repopulate draft ALSO states the module's
difficulty (and the party level), through the same clause composer, and the
in-place check reads the encounter's own level.
...
- **pf2e — the NO-NUMBERS rule is now a CHOSEN policy (`'verbatim'`), not a
  system hard-code** (docs/17 row 180): creature budgets are Paizo's (GM
  Core). Under `'verbatim'` Campaigner ships NO numeric pf2e budget; the
  prompt directs the model to the retrieved GM Core excerpts VERBATIM when
  they are present. Whether an excerpt actually surfaced is not
  deterministically decidable from the retrieval output, so the deterministic
  check is replaced by an ALWAYS-ON advisory (`PF2E_BUDGET_ADVISORY`)
  persisted on the run and the artifact — over-loud by design, never a
  fabricated paraphrase (AGENTS rule 1). `'system'` maps a pf2e module here
  (the legacy reading), and a pf2e module created under the new default
  instead runs `'pf2e-budget'` (the amendment below).
...
### D12 amendment — the selectable budget policy (docs/17 row 180, owner-requested)

Owner, verbatim: *"please make that policy selectable during module creation
with a sensitive default. … Please Implement this now and make the policy
selectable in the module generator dialog."* ("sensitive" is read as SENSIBLE:
the default must produce a real, level-appropriate challenge.) The owner's
report was *"only 1 mob per room and they are easy mobs. No challenge at all
for a group. Repopulate does not help"*, on a Dungeon at fill grade 75,
PF2E — and the measured cause is that `'verbatim'` was a system HARD-CODE, so
the displayed fill grade never became a numeric target.

- **The seam (ONE value, threaded once).** `src/domain/encounterBudget.ts`
  owns the persisted, zod-validated policy; `roomBudget.encounterBudgetFor`
  resolves it into the run's ONE `EncounterBudget`
  (`{ policy, mode, scale, repairUnder }`), and `runEngine` resolves that ONCE
  per run and passes it to the Cartographer brief, the per-room check, the
  stocking cap, the repopulate/finalize paths and the in-place fill. No call
  site branches on the system again. `encounterPartyLevel` is the ONE party-
  level resolver the roster WINDOW and the brief share.
- **The three values.** `'system'` — the compatibility mapping (the dnd5e
  numeric band; verbatim for pf2e); a module row without the field reads here,
  so legacy rows are byte-identical. `'pf2e-budget'` — the fill grade is the
  share of Campaigner's OWN documented pf2e approximation
  (`pf2eStandardThreatLevels`: `2 × party level` creature-levels for a party
  of four, scaled by party size — deliberately NOT the dnd5e `T + 2` band),
  and an under-strength room is a REPAIRABLE issue; retrieved GM Core excerpts
  are still obeyed VERBATIM in the prompt. `'verbatim'` — ship no numbers,
  today's pf2e behaviour byte-identical, kept selectable because shipping no
  Paizo numbers is a deliberate licensing stance.
- **The default is sensible and visible.** A `pathfinder2e` module defaults to
  `'pf2e-budget'`; every other system to `'system'`. The New Module dialog
  shows it and passes the choice through the persisted `newModuleDraft` seam;
  `createModuleAndRun` STAMPS it on the module row, so every later generation
  and repopulate of that module reads the row deterministically (no drift when
  settings change later). `moduleSchema.encounterBudgetPolicy` is additive
  optional (`.nullable().default(null)`, parse-on-read, no Dexie version).
- **Licensing boundary.** No Paizo table is embedded anywhere. The pf2e path
  persists the LOUD `PF2E_APPROXIMATION_ADVISORY` on every run (the digit-level
  companion of the always-on verbatim advisory), and the dnd5e band path is
  byte-identical under `'system'`.

### D12 amendment — the module difficulty setting (docs/17 row 190, owner-requested)

Owner, verbatim: *"I would like to have a difficulty setting in the module
creation dialog. 5 steps, with the middle one as normal difficulty. Such that i
can adjust a module for group strength."*

Difficulty is the SIBLING of the budget policy above, not a second version of
it. The policy answers *which rule bounds a room's challenge* (`'system'` /
`'pf2e-budget'` / `'verbatim'`); difficulty answers *how hard the module should
be for this group* and SCALES whatever numeric budget that rule produces. They
are independent settings: `'verbatim'` + `'much-harder'` is a legal pair, and
folding them into one enum would make the licensing stance and the tuning
mutually exclusive.

- **The seam (ONE value, threaded once).** `src/domain/moduleDifficulty.ts`
  owns the five steps (`much-easier`, `easier`, `normal`, `harder`,
  `much-harder`), the zod enum, the label map, the default (the middle step)
  and the ONE resolver; `roomBudget.encounterBudgetFor(policy, system,
  difficulty)` carries the resolved difficulty on the run's ONE
  `EncounterBudget`, resolved from the owning module at the same three sites
  that resolve the policy. The module row field is
  `moduleSchema.difficulty` (`.nullable().default(null)`, parse-on-read, no
  Dexie version): an unset row is distinguishable from an explicit `'normal'`
  and legacy rows resolve to normal.
- **What the multiplier moves — and the ONE place it moves it.** The ladder is
  `0.5 / 0.75 / 1 / 1.5 / 2`: the middle is today's numbers, the ends exactly
  halve/double the standard budget, the adjacent steps are ±25–50%, and it is
  monotone and round so the scaled band stays legible in prompts and
  advisories. `roomBudgetBandUpperFor` — the ONE function that computes a
  room's standard-encounter number for BOTH scales — multiplies its base by
  `difficultyBudgetMultiplier`, so the `'over'` verdict, `expectedRoomThreat`
  (the fill-grade `'empty'`/`'under'` expectations), the complex stocking cap
  and the numbers the brief states all move together. `fillGradeStockingFor`
  reads that same seam now (its hand-spelled `pf2eStandardThreatLevels` call
  was folded in). The Cartographer brief carries a `MODULE DIFFICULTY` clause
  naming the owner's step; at anything but normal it states the SCALED band,
  so the model aims at the numbers the deterministic check uses. The clause
  explicitly separates the module setting from the reply's own per-encounter
  `difficulty` string.
- **The `'verbatim'` answer, stated honestly.** No numeric budget is computed
  under `'verbatim'`, so there is nothing for the multiplier to scale: the
  clause is a DIRECTION (no digit) and the no-numbers licensing stance is
  untouched. The multiplier applies only where numbers are computed at all.
- **Licensing boundary.** The multipliers scale Campaigner's own approximations
  only (the dnd5e `T + 2` band, the pf2e `2 × party level` approximation). No
  GM Core table and no DMG table is quoted, paraphrased or restated, and no
  licensed number is embedded.
- **Scope.** The setting is chosen at CREATION (the New Module dialog's
  `Advanced — encounter guardrails` disclosure, five steps, middle selected by
  default) and recorded on the row; there is deliberately NO editable-after-
  creation control in this slice. Editing a module's difficulty later would be
  a `patchModule` write of the same field plus a re-run, not a new seam.
...
- **Party level** is the referencing part's level: at brief time the
  ...
  count.
- **Every prompt carries the one structured line** `Party of 4 adventurers at
  level N.` (`partyLevelLine` — the ONE composer; the earlier claim that this
  route list was complete was wrong and docs/17 row 228 corrected it): the
  FRESH Smith stub draft via `buildEntityBrief`
  ...
  `surroundingParagraphs` excerpts); the Cartographer brief via
  `runEncounterBrief` (module lookup through the target's `moduleId`, same
  helper); and the TARGETED single-room repopulate Smith draft via
  `runDraft`'s own read (`runEngine.encounterInputGuidanceFor`) — the route
  that previously sent NO level at all, which is the reported level-9-mobs
  bug (docs/17 row 228). All routes resolve through the SAME
  `encounterPartyLevel` chain and render the SAME `partyLevelLine`.
