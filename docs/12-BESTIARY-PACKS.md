...
**AMENDED 2026-09-17 by docs/17 row 189** — the pf2e bestiary adapter now stamps
the creature's own `items[]` of type `spell` onto `statBlock.spells`, so an
imported library caster renders the SAME chips an AI-authored mob does through
the ONE `components/spell-chip.SpellChip` / `features/spells/mob-spell-chips`
path (row 184). The importer resolves **nothing**: it stamps the source's own
names verbatim, and the loud unresolved report stays with the ONE resolver
