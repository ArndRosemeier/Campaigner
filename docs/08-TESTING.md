...
**NO PIN IN THIS SUITE SEES LAYOUT.** jsdom computes no boxes, so overlap,
collision and "does the dialog fit a 768x1024 iPad" are STRUCTURAL pins at
best: `docs/17` row 336's spawn-dialog fix asserts which element scrolls, which
classes carry the truncation/shrink contract and where the header sits — never
pixels. Layout work of that kind owes a real-device check, and the test file
says so. `docs/17` row 339's Core-mobs pins go one step further only because the
VIRTUALIZER does the arithmetic itself: they stub the row's
`getBoundingClientRect` and require the row PITCH and the scroll track to follow
the MEASURED height — still the virtualizer's arithmetic over a STUBBED box,
never a real one, so it too owes the device check.
...
| The spawn dialog fits a small viewport and its pick rows cannot collide (docs/17 row 336, defects B and C): the header, search field and BOTH illustrate ticks are outside the only scroller in a viewport-bounded dialog; every pick row's label truncates and its action is `shrink-0` | `spawn-picker-illustrate.test` (2 STRUCTURE pins) | ✅ REVERT-PROVEN (restoring the pre-336 layout REDs both by name; `SpawnPicker.tsx` `e02f55a9…` → `68572d77…` → restored byte-identically) — **AND THE HONEST LIMIT: jsdom computes no layout, so NO pin in this suite can observe the owner's real overlap or the pixels of a 768x1024 iPad dialog. These pins assert the structure the fix turned on; the visual result OWES a real-device check.** |
