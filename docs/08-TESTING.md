...
with a `vh` fallback. **jsdom cannot scroll by touch or momentum, so NOTHING here
can prove the dialog scrolls on his iPad — that check is OWED to the owner and
the fix is never reported as proven.**
