...
**NO PIN IN THIS SUITE SEES LAYOUT — AND NONE CAN SCROLL BY TOUCH.** jsdom
computes no boxes, so overlap, collision and "does the dialog fit a 768x1024
iPad" are STRUCTURAL pins at best: `docs/17` row 336's spawn-dialog fix asserts
which element scrolls, which classes carry the truncation/shrink contract and
where the header sits — never pixels. Layout work of that kind owes a real-device
check, and the test file says so. `docs/17` row 339's Core-mobs pins go one step
further only because the VIRTUALIZER does the arithmetic itself: they stub the
row's `getBoundingClientRect` and require the row PITCH and the scroll track to
follow the MEASURED height — still the virtualizer's arithmetic over a STUBBED
box, never a real one, so it too owes the device check. `docs/17` row 340 is the
case that makes the limit sharpest: the owner's report is *"Spawn dialog now does
not scroll anymore on my ipad"*, and its pins can assert only that the dialog has
ONE scroller (the body), that the virtualizer's scroll listener lands on that
body, that its visible window is computed in the body's CONTENT coordinates (the
track's offset is the `scrollMargin`, stubbed), and that the cap declares `svh`
with a `vh` fallback. `docs/17` row 343 goes one step further and it is the
sharpest limit of the set: the owner then reported *"Still can't scroll an I also
see no scroll bar"*, and the working diagnosis is a WebKit layout failure (a
`max-height`-only ancestor that does not bound a `flex-1 min-h-0` child, so the
content is clipped and never overflows). Its evidence is an in-repo CONTROL —
`HelpDialog` ships the same body under a DEFINITE `h-[80vh]`, and the height TYPE
is the only difference from the failing picker — which is strong evidence and
still an inference rather than a measurement. **NOTHING in this suite can observe
it: jsdom computes no layout, so it cannot see clipping, overflow or a scrollbar,
and it cannot scroll by touch or momentum. The pins assert only the STRUCTURE (a
definite `h-…` class on the dialog and USED, ONE scroller, the virtualizer
observing it, the broken `max-h` shape refused); the iPad check that would
establish the DIAGNOSIS as well as the fix is OWED, and neither is ever reported
as proven.** The source-level half lives in
`tests/architecture/one-dialog-viewport-box.test.ts`, which is a spelling, shape
and population assertion, not a layout one.
...
| **The description discipline: NPC, location and faction descriptions carry DETAIL, not STORY** (docs/17 row 365): ONE exported clause rides EXACTLY `npc-smith`, `worldbuilder` and `faction-designer` (a fourth carrier, or a missing one, reds by name), each exactly once immediately before the JSON contract; every existing grounding/context sentence is byte-unchanged (the pin reconstructs each pre-change prompt by removing one clause and requires byte equality with a golden captured before the landing); and all SEVEN exempt personas — including the two `note` writers the owner kept out of scope — are byte-identical to it | `personaDescriptionClause.test.ts` (17 pins, over `tests/fixtures/personaDescriptions/pre-change-prompts.json`) | ✅ REVERT-PROVEN (four arms, each RED by name, every injected hash printed and DIFFERENT, the source restored byte-identically: the clause dropped from `npc-smith` → the carrier + exactly-once pins; the clause added to exempt `continuity-editor` → the carrier + byte-identity + "none carries it" pins; the Worldbuilder grounding sentence removed → the ADDITIVE + grounding pins; the reason reworded → the reason pin; raw arms `.gate-logs/row365/arms/`) |
...
| **A player is never coverage-hidden by a veil or a fog** (owner's guarantee, docs/17 row 331): the REAL producer's covered set holds the NPC under BOTH kinds and nothing else; player view removes the covered NPC while the PC, a statless token and a stamp stay, and the GM view shows every token; every veil of both kinds precedes every token in document order | `battle-surface.test` (`players are never coverage-hidden (row 331)` — 3 pins; the producer pin mounts the real `useBattleState`, never a re-implemented predicate) | ✅ REVERT-PROVEN (three arms, each RED by name, every source restored byte-identically: the kind filter dropped → the producer and player-safe pins; the two layers swapped → the order pin; the removal made unconditional of `playerSafe` → the GM half; raw arms `.gate-logs/row331/`) |
...
| **The board has ONE writer, and a spawn cannot be erased by a later board write** (docs/17 row 336, defect A): the seam applies the caller's mutation to the row read INSIDE its own transaction (a token written after the caller's snapshot survives; two concurrent mutations both land; a snapshot-derived seam loses one); the surface's own commit and the reconcile's newcomer/prune branches keep a REAL spawn that lands while they are in flight; `patchBattle` refuses a board at the TYPE level (`@ts-expect-error`, enforced by the compile tier) and the source scan names the exact writer populations | `battleRepo.test` (the board-mutation seam: 4 pins incl. the type arm), `battle-surface.test` (`the owner’s vanishing mob` + `the reconcile’s prune`), `one-battle-board-write.test` (SOURCE SCAN) | ✅ REVERT-PROVEN (two arms, each RED by name, restored byte-identically: `BattleSurface.tsx` `5e2e64be…` → `e65f5dbb…` snapshot-derived commit → BOTH surface pins red; `battleRepo.ts` `79b06554…` → `f96dec4e…` seam reading outside the transaction → the concurrent pin red) |
| **A board the scrub empties is DELETED — the ROW is gone, and the empty rule is decided on the board the scrub produced** (docs/17 row 336 fix-forward): the seam carries a DELETE outcome, decided in the same transaction from the mutation's own board; a campaign PC artifact on a board holding only a doomed npc token does not keep the row alive (normalize-on-write re-ensures that PC token, which is why a read-back decision failed) | `battleRepo.test` (`DELETES a board the scrub empties out — the ROW IS GONE…`), `deleteArtifactsOfKind.test` (the kind + selection cascades: census and executed pass agree), `one-battle-board-write.test` (the deletion site is the seam's) | ✅ REVERT-PROVEN (two arms, RED by name, restored byte-identically: `battleRepo.ts` `c2fffc97…` → `9e175afe…` scrub deciding from the SAVED row again → the row pin + the two cascade tests; `c2fffc97…` → `9e18993d…` the seam ignoring the `null` outcome → 4 red) |
| The spawn dialog fits a small viewport and its pick rows cannot collide (docs/17 row 336, defects B and C; the scroller half is extended by row 340): the header, search field and BOTH illustrate ticks are outside the only scroller in a viewport-bounded dialog, and NO element inside the dialog other than the body carries an `overflow-auto`/`overflow-y-auto`/`overflow-scroll` box; every pick row's label truncates and its action is `shrink-0` | `spawn-picker-illustrate.test` (2 STRUCTURE pins) | ✅ REVERT-PROVEN (restoring the pre-336 layout REDs both by name; `SpawnPicker.tsx` `e02f55a9…` → `68572d77…` → restored byte-identically) — **AND THE HONEST LIMIT: jsdom computes no layout, so NO pin in this suite can observe the owner's real overlap or the pixels of a 768x1024 iPad dialog. These pins assert the structure the fix turned on; the visual result OWES a real-device check.** |
| The illustrate tick is offered in the author section too, bound to the SAME flag and the same illustrate path (docs/17 row 336, defect B) | `spawn-picker-illustrate.test` (2), `spawn-picker-author-mob.test` (2) | ✅ REVERT-PROVEN (removing the author-section control REDs 3 by name; `SpawnPicker.tsx` `e02f55a9…` → `674074a8…` → restored byte-identically) |
| **The Core-mobs VIRTUALIZED rows are MEASURED, never sized by the estimate, and a dialog cap survives a browser without `dvh`** (docs/17 row 339; owner, verbatim: *"Core mobs also have overlapping spawn buttons."*): the row PITCH and the scroll track follow the MEASURED row height (a stubbed 60px rect), never the 44px estimate; each row carries a `minHeight` FLOOR and no forced `height`, so the measurement cannot be self-fulfilling; every Core-mobs label carries `whitespace-nowrap` + `truncate` beside a `shrink-0` action; the dialog declares `max-h-[85vh]` on a `vh` fallback (row 340 later replaced the `dvh`-only override with `supports-[height:100svh]:max-h-[min(85svh,85dvh)]`, and row 343 then replaced the whole declaration with the definite `DIALOG_VIEWPORT_BOX` — this pin asserts whatever the dialog's height declaration is, and the CURRENT shape is pinned by the row-343 entries), and the shared `DialogContent` base cap carries the same `vh` fallback | `spawn-picker-illustrate.test` (4, the row-339 `describe`), `ipad-chrome.test` (1) | ✅ REVERT-PROVEN (FIVE arms, each RED by name, no two arm hashes identical, all four files restored byte-identically: the measurement wiring dropped → the pitch pin; `height: item.size` restored → the floor pin; `whitespace-nowrap` dropped → the label pin; each cap's `vh` fallback dropped → its own pin; raw arms `.gate-logs/row339/arms/`) — **AND THE HONEST LIMIT: jsdom computes no layout, so these pins can see only the VIRTUALIZER's arithmetic over a STUBBED rect — never the owner's real overlap or the pixels of a 768x1024 iPad. The visual result OWES a real-device check.** |
