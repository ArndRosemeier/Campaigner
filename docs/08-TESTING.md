...
| The raw-token carrier has NO route into an export: the module/deliverable definition is bracket-free while the display text IS present (and the tokens proven present on the rows first), and a source scan proves `lib/modulePdf` / `lib/pdfExport` / `lib/mdToPdfmake` never import `WikiMarkdown` / `remark-wikilinks` / the carrier constant. Also RECORDS the pre-existing opposite behaviour of the single-artifact path (`markdownToText` is not wiki-aware, so it prints the token verbatim — reported, not fixed) | `wiki-raw-export.test` (3) | ✅ |
