...
GUARDED delete. The OFFER is guard-aware and truthful (docs/17 rows 92 and
323): the panel's read-time derivation calls the sweep's own
`evaluateOrphanGuards` with the subset of the guard bundle its props can see
and composes it with the refusals a sweep returned; a refused row is held IN
USE and is NOT reported at all, so the group — and the count, the button and
the confirm — name only rows a sweep will actually delete. The delete gate is
the campaign-wide version of this spec's
