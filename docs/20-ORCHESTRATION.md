...
```
reconciled: 5ca23fb · 2026-09-24T10:50Z (ROW 343 LANDED — the iPad scroll fix-forward, ONE definite-height seam at the 3 dialogs whose body is the scroller, and the dispatcher's OWN arms are RED by name. THE INTEGRATED GATE OVER 340+341+343 WAS RED on two pins, and row 344 fixes BOTH forward: this row records the RED, not a green. IN FLIGHT: row 344, then the union re-gate in full. OWED: the owner's iPad check, which is the only real proof.)
...
IN-FLIGHT | row=344 | state=DISPATCHER FIX-FORWARD | note=THE INTEGRATED GATE OVER 340+341+343 WAS RED: two pins, one per chunk. (1) row 341 added an 8th `resolveChatModel(` site in `runEngine.ts` without updating the row-198 population pin — only the FULL gate could see it; the site is CLASSIFIED, not count-blessed. (2) row 336's interleave pin used a fixed `flushAsyncUpdates(30)` BUDGET, not a wait for the commit, so it measured machine speed. Both green now (2 files / 125). Detail: row 344.
...
IN-FLIGHT | row=343 | state=LANDED-PENDING-GATE | sha=5ca23fb | writer=698761ae (finishing) | note=the iPad scroll fix-forward: ONE definite-height seam (`DIALOG_VIEWPORT_BOX` + `DIALOG_SCROLL_BODY`) at all 3 dialogs whose body is the scroller; the base cap KEEPS `max-h`. THE DISPATCHER'S OWN ARMS ARE RED BY NAME (breaking the seam's value, and a consumer dropping it) with byte-identical restores. OWED: the union gate (RED once, see row 344) and the owner's iPad check, which is the only proof.
