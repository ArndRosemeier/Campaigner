...
```
reconciled: 2b8abda · 2026-09-21T19:55Z (row 304 LANDED — a legacy spell payload heals at the library read boundary; the dispatcher's OWN integrated gate is GREEN over the gated tip. NO writer in flight, no worktree, no branch but `main`; the owner's module-scroll request is reserved as row 305 and awaits one answer)
...
SESSION  | cos=session-b276401e-48d8-4c02-b2fb-7494264a77da | started=2026-09-20 | model=deepseek-official/deepseek-flash | state=LIVE (owner-designated 2026-09-21) | note=reconciled at `0b2b2b2`; landed rows 274 (session-record preset trap), 275 (commit-sweep incident) and 304 (legacy spell payload heals at the read), plus the AGENTS commit-pathspec guard (`07f227b`). Holds ONE standing goal, PAUSED; wake events only.
...
QUEUE | row RESERVED 305 | SMALL-MEDIUM — SCOPED, one answer needed | THE MODULE READER LOSES ITS SCROLL (owner 2026-09-21). PROVEN: `ModuleReaderPage`'s scroll div unmounts when an `[[Encounter]]` chip opens the battle, remounting at 0; the text is re-read, never regenerated. Scope: ANY navigation, SESSION-ONLY. Plan: an in-memory module-keyed memory per `previewStore`; keep-alive and persisted-per-module REJECTED. Report `discoveries/module-scroll-memory.md`. OPEN: pixel vs `#part-<n>`.
