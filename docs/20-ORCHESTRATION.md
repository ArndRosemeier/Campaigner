...
QUEUE | row RESERVED 339 | IN FLIGHT — owner defect | THE CORE-MOBS SPAWN ROWS STILL OVERLAP on iPad: row 336 excluded the VIRTUALIZED rows from its wrap fix; the owner reports the exclusion IS the defect. Two candidate causes to decide between (a wrong virtualizer height estimate, or `dvh` unsupported on older iPads), then a structural fix (measure the real height, nowrap + truncate, shrink-0, a `vh` fallback) with the owed real-device check RECORDED, never claimed.





IN-FLIGHT | row=339 | state=DISPATCHED | writer=b86bbe0f-85fe-4f25-8db4-a9e2371bb5f6 | worktree=/home/administrator/projects/Campaigner/worktrees/mobs-layout | branch=mobs-layout | model=deepseek-official/deepseek-flash (inherited route) | note=Core-mobs row overlap on iPad; brief `.gate-logs/briefs/row339.md`; reports ON LANDING or BLOCKED only.
