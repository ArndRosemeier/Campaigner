...
QUEUE | row RESERVED 328 | IN FLIGHT — owner | A BATTLE ADOPTS THE ENCOUNTER’S CURRENT MAP: heal on open when the board has NO map (nothing to clobber), plus an explicit “Use the encounter’s current map” on a live board behind a confirm (map AND layout move, tokens stay); the regeneration toast stops naming an action that does not exist. Owner, verbatim: *“regarding the battle map, do what your suggestion is.”* Auto-converging live boards stays REJECTED.

IN-FLIGHT | row=327 | state=LANDED-PENDING-GATE | sha=90e25596 | writer=cf87e1b0 (retired) | model=deepseek-official/deepseek-flash | note=module-owned rows got the checkbox (export/reuse); its worktree, branch and session are RETIRED after the LANDING report, and the integrated gate over 327+328 is the pending tier.
IN-FLIGHT | row=328 | state=DISPATCHED | writer=4fd866fe-5fa5-41cb-8b2f-ef1315e05f50 | worktree=/home/administrator/projects/Campaigner/worktrees/battle-map | branch=battle-map | model=deepseek-official/deepseek-flash (inherited route) | note=battle adopts the encounter’s current map (heal on open + explicit action); brief `.gate-logs/briefs/row328.md`; reports ON LANDING or BLOCKED only.
