...
```
reconciled: 715942d · 2026-09-23T22:22Z (ROWS 333+334 GATE GREEN over the union: battle spawning gains the OFF-by-default illustrate checkbox and the NPC-Smith author-and-spawn (stat block or nothing), and the party wizard onboards a party with two questions per character. GATE GREEN: 378 files / 4827 tests, 7/7 chunks, lint 0, peak 2578MB, 623s. OPEN: QUEUE 311 (six owed pins), 318, 321, 330 (the ledger table-integrity pin); the level-9 encounter report stays closed-unreproducible.)
...
QUEUE | row RESERVED 336 | IN FLIGHT — owner defect | THREE SPAWN DEFECTS: (A) a spawned mob appears then vanishes on the next redraw — `BattleSurface.commit` writes a board from the RENDER SNAPSHOT while `saveBattleBoard` replaces it wholesale, so the reconcile the spawn re-triggers clobbers it; fix = ONE read-modify-write mutation seam in `battleRepo`; (B) the illustrate checkbox lives only in the dialog header, not the author section; (C) the rows and dialog collide on a short iPad viewport.
QUEUE | row RESERVED 337 | IN FLIGHT — owner | EMPTY BATTLEMAP: the prompts already ban characters/monsters (not a guarantee), so the `vision-map` step that already sends the map to a vision model gains a REQUIRED `figures` answer; a non-empty answer FAILS the step loudly and finalizes NOTHING. Both prompts gain the positive "empty terrain; creatures are tokens added later" sentence. GAP: the CLASSIC path has no vision read; enforcing it there costs one vision call per map — the OWNER’S call.




IN-FLIGHT | row=337 | state=DISPATCHED | writer=2ee643aa-cab7-4b5c-8bfd-9780cafdde85 | worktree=/home/administrator/projects/Campaigner/worktrees/map-empty | branch=map-empty | model=deepseek-official/deepseek-flash (inherited route) | note=empty-battlemap enforcement in the vision read + positive prompt framing; brief `.gate-logs/briefs/row337.md`; reports ON LANDING or BLOCKED only.
IN-FLIGHT | row=336 | state=GATE RED, FIXING FORWARD | sha=def8ed5 | writer=8fec753e | model=deepseek-official/deepseek-flash | note=the full gate over the landing went RED: the removal census PREDICTS an emptied board deleting itself (1) while the executed pass MEASURES 0 — an emptied board is no longer deleted (a seam-fold regression). Fix-forward at the seam, with a row-gone pin. OWED from the owner: CORE-MOBS rows still overlap on iPad (excluded from the wrap fix) — next slice.
...
LANDED | row=327 | sha=90e2559 | verify=GATE GREEN — union gate over 327+328 (row 328). MODULE-OWNED ROWS ARE SELECTABLE: the module group mounts ONE `ArtifactTreeRow`, the same component the campaign sections use; the selection resolves over every live row, so a module NPC exports and imports at campaign level elsewhere. REMOVE still refuses module rows by name; Library stays unselectable. Arm: adding library rows to the resolution stayed GREEN → sixth owed pin (QUEUE 311).
