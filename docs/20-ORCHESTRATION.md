...
```
reconciled: f42626d · 2026-09-24T11:05Z (UNION GATE GREEN over rows 340+341+343+344 — 381 files / 4866 tests, 7/7 chunks, lint 0, peak 2619MB, 628s. The FIRST union run was RED on two TEST-side pins and row 344 fixed both forward rather than re-running until it looked green. Row 343's worktree, branch and session are retired; no writer is in flight. OWED: the owner's iPad check — jsdom cannot scroll, so the mechanism rests on the in-repo HelpDialog control and stays INFERRED until he tries it.)
