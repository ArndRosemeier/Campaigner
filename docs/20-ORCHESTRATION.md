...
```
reconciled: 498f1b1 · 2026-09-25T08:40Z (ROWS 357+358 GATE GREEN — a module version CAPTURES and RESTORES the premise, so "make undoable" holds, and the adversarial pass runs INSIDE generation when the flag is on: premise first, then each part, with flag-off byte-identical to a golden transcript. Both stale comments were fixed forward. ROW 360 IN FLIGHT: the chat trigger, the last slice. OPEN: the iPad scroll check, the itemization go-ahead (347), the stylize retry (348), the owed pin 351.)
...
IN-FLIGHT | row=360 | state=DISPATCHED | writer=e3d3883a-f96d-44da-a4e0-ddb931dbfdd9 | worktree=/home/administrator/projects/Campaigner/worktrees/adversarial-chat | note=THE CHAT TRIGGER (owner: "triggerable in the module chat"): extend the ONE `<change>` seam so a chat turn runs the EXISTING pass on the PREMISE or a PART, rendering the CRITIQUE FINDINGS AND the edit in the chat's existing surfaces. Accepting stays the existing path (undoable); no new overlay or action path.
...
LANDED | row=344 | sha=f42626d | verify=GATE GREEN — the same union gate as row 343. TWO pins the FIRST union run went red on, both TEST-side: row 341 added an 8th `resolveChatModel(` site without updating the row-198 population pin (CLASSIFIED — a run step reached only via the funnel — not count-blessed), and row 336's interleave pin waited a fixed `flushAsyncUpdates(30)` BUDGET instead of for the commit, so a busy box could fail it; it now waits for the condition.



LANDED | row=343 | sha=5ca23fb | verify=GATE GREEN — the union gate over 340+341+343+344: 381 files / 4866 tests, 7/7 chunks, lint 0, peak 2619MB, 628s, log `.gate-logs/gate-3-20260924T085008-9194`. ONE definite-height seam (`DIALOG_VIEWPORT_BOX` + `DIALOG_SCROLL_BODY`) at the 3 dialogs whose body is the scroller; the base cap KEEPS `max-h`; a source pin REFUSES the old shape. ROW 340 DID NOT CURE THE SCROLL (kept red). Cause: the in-repo `HelpDialog` control, INFERRED — the iPad check is OWED.
LANDED | row=341 | sha=a018460 | verify=DISPATCHER ARM RED BY NAME — removing the classic figure check made the NAMED classic-refusal pin fail (`expected 'completed' to be 'failed'`); pristine/restored `runEngine.ts` hashes match (`3272154a…`). GATE GREEN in the union run (row 343). ONE client factory serves BOTH map paths; the check runs on the RAW blob BEFORE intake/store; a vision failure propagates. HONEST LIMIT: a vision read is a MODEL judgement, not a pixel proof.
