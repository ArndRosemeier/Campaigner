...
```
reconciled: 5074450 · 2026-09-21T22:20Z (row 308 gated GREEN after its own lint RED was fixed forward; the owner then settled the ASSERTION design — prose assertions are ABSOLUTE, transcribed by the reading model and enforced, exempt from the budget — reserved as row 309 and in flight. No other writer; the 308 writer is retired in this landing's wake)
...
SESSION  | cos=session-b276401e-48d8-4c02-b2fb-7494264a77da | started=2026-09-20 | model=deepseek-official/deepseek-flash | state=LIVE (owner-designated 2026-09-21) | note=reconciled at `0b2b2b2`; landed rows 274, 275, 304, 305, 306, 307 and 308, plus the AGENTS commit-pathspec guard (`07f227b`). Dispatched row 309 (asserted cast is absolute). Holds ONE standing goal, PAUSED; wake events only.
...
QUEUE | row RESERVED 309 | MEDIUM — IN FLIGHT | ASSERTED CAST IS ABSOLUTE (the DECIDED-OWNER entry above): transcribe the scene's asserted cast, surface + store it, require presence (repair then fail), EXEMPT it from the budget, and close the substitution hole for asserted figures. Evidence/recipe: `.gate-logs/discoveries/encounter-composition.md`.
...
QUEUE | row RESERVED 311 | SMALL — batched with the next code slice | TWO OWED PINS: a statless PC with a NON-ZERO `initiativeOverride` (row 308's pin asserts 0, so it cannot distinguish the override — the dispatcher's arm left it GREEN), and the deep-link reload into a live board that still misses a newly created player (docs/18 §5).
