...
```
reconciled: 9831eb1 · 2026-09-24T12:05Z (ROW 349 — SPAWNED CORE MOBS ARE STATLESS: the clean-cut purge rewrote a roster row's library CITATION into a name-only entry, so the spawn mints a token with no HP or initiative while the library STILL HOLDS the creature. THE FORK IS HIS: a repair copying the stats onto name-only rows (rec.) vs resolving the library at spawn time (rejected). OPEN: the iPad scroll check, the itemization go-ahead (347), the stylize retry (348), this repair (349).)
...
QUEUE | row RESERVED 349 | OWNER DEFECT — FORK TO HIM | SPAWNED STANDARD CORE MOBS ARE STATLESS: the clean-cut purge rewrote a roster entry's library CITATION into `{ type: 'none' }` (name-only, `cleanCut.ts:94-96`), which resolves to `statBlock: null` and mints a statless token with no seed row. Fork — (A, RECOMMENDED) a REPAIR copying stats onto `none` entries whose name resolves, riding the EXISTING `db/mobCopyRepair` seam; (B) spawn-time library resolution, REJECTED; (C) leave it, REJECTED.
AWAITING-OWNER | id=statless-core-mobs | cost=1 slice | question=Your spawned core mobs are STATLESS because the clean-cut purge turned a roster row's library CITATION into a name-only entry: the spawn mints a token with no HP or initiative, though the library holds the creature. RECOMMENDED: a REPAIR copying the library stats ONTO name-only rows whose name resolves — a COPY, never a pointer. REJECTED: resolving the library at spawn time. With the pack gone the row stays statless. Go-ahead?
