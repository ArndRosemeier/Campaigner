...
```
reconciled: a241c3e · 2026-09-24T11:35Z (ROW 346 DISPATCHED — the illustrator must prioritize direct instructions over context, and MEASURED it currently DROPS the owner's instruction silently: the image step passes only raw `extraInstruction`, so the brief's `Additional instruction:` never reaches the composer. See the IN-FLIGHT row. 347 reserves the itemization arc, awaiting the go-ahead. UNION GATE GREEN at f42626d; the iPad check is the one owed proof.)
...
IN-FLIGHT | row=346 | state=DISPATCHED | writer=c53b901b-2e2d-4709-a6ed-fb07fb5c9aa7 | worktree=…/worktrees/illustrator-instruction | note=THE ILLUSTRATOR MUST PRIORITIZE DIRECT INSTRUCTIONS OVER CONTEXT. MEASURED: the image step passes only raw `extraInstruction`, EMPTY on a fresh run, while his text rides the brief's `Additional instruction:` paragraph that `directInstructionFor` extracts — so it is DROPPED SILENTLY. Fix = reach + ONE precedence clause. Brief: row346.md.
