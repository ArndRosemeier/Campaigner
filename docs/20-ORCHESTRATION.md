...
```
reconciled: bb447aa · 2026-09-22T20:22Z (THE IMAGE TEXT RULE IS POSITIVE (row 319): the shared avoid list is DELETED, the default prompt carries NO `Avoid:` line, and one positive clause says text where needed is welcome. GATE GREEN: 373 files / 4772 tests, 7/7 chunks, lint 0, peak 2473MB, 583s. The duplicate-fold arc stays complete (rows 312–316, `src/` inventory EMPTY), its tool limit recorded. OPEN: the level-9 encounter diagnosis (awaiting a backup/export); QUEUE 311 + 318 pins.)
...
QUEUE | row RESERVED 322 | IN FLIGHT — owner | ARTIFACT SELECTION IN THE WORKSPACE: multi-select with Export selected (existing path + `selectionOnly`, so it does not drag the campaign's modules/battles/runs), Remove selected (NEW id-set seam, live census + confirm; Party refused) and Import… into the CURRENT campaign (new target-campaign mode; rows land at campaign level, `moduleId` null). Owner: *"not on the artifact card, but inside the workspace … so multi exports get easy."*
QUEUE | row RESERVED 323 | IN FLIGHT — owner | AN IN-USE ENTITY IS NOT REPORTED: the module panel's orphan group lists ONLY the deletable rows (unmentioned AND unused) — a row the guards keep (cross-module mention, encounter roster, battle token/seed) is no longer shown with its `in use — …` reason. Owner: *"if something is used, do not report on it. Genuinely unmentioned and unused things are good to report."*

IN-FLIGHT | row=322 | state=DISPATCHED | writer=9cf0803a-c55d-4994-9f15-d9b04ac5be3f | worktree=/home/administrator/projects/Campaigner/worktrees/sel-ws | branch=sel-ws | model=deepseek-official/deepseek-flash (inherited route) | note=workspace selection: export/remove/import; brief `.gate-logs/briefs/row322.md`; reports ON LANDING or BLOCKED only.
IN-FLIGHT | row=323 | state=DISPATCHED | writer=c392b94f-e1ad-434e-af9b-235962710606 | worktree=/home/administrator/projects/Campaigner/worktrees/orphan-copy | branch=orphan-copy | model=deepseek-official/deepseek-flash (inherited route) | note=in-use rows leave the orphan group; brief `.gate-logs/briefs/row323.md`; reports ON LANDING or BLOCKED only.
