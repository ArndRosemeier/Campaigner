...
```
reconciled: 18165fa · 2026-09-15T10:40Z
...
IN-FLIGHT | row=172 | branch=feat/dup-tripwire | worktree=/tmp/campaigner-dupes | base=7b390de | writer=session-ac0d5f92-8df7-4560-8053-1e4b7e0de869 | state=running | note=the duplicate-body tripwire AGENTS §Centralization obligation 4 names as owed (owner-ordered enforcement); its baseline captured the seven `isRecord` copies that row 171 folded, so those entries are STALE at rebase time and the CoS removes them — the staleness check doing its job
