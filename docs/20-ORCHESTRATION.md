...
```
reconciled: 9c5440c · 2026-09-24T07:45Z (ROW 339 GATE GREEN — the Core-mobs rows are MEASURED, not forced to the 44px estimate, so the iPad overlap (a px literal against rem content scaled by --ui-scale) is gone. THE RED IS KEPT: it introduced an iPad scroll regression — row 340 fixes the scroll chain (ONE scroller + a small-viewport cap) and row 341 verifies the CLASSIC map path for figures. GATE GREEN: 379 files / 4852 tests, 7/7 chunks, lint 0, peak 2516MB, 622s.)
...
QUEUE | row RESERVED 343 | IN FLIGHT — owner defect | THE SPAWN DIALOG *STILL* DOES NOT SCROLL on iPad with MANY SCREENS OF MOBS. Row 340 did not cure it: content overflows, NO scrollbar, a drag does nothing. Cause (INFERRED, not device-measured): the scroller moved off the capped DIALOG (which scrolls fine on WebKit) onto a `flex-1 min-h-0` CHILD under a `max-height`-only parent WebKit does not bound, so it clips. Fix = a DEFINITE height (`h-fit` preferred, `h-[85vh]` fallback), ONE scroller.
QUEUE | row RESERVED 341 | IN FLIGHT — owner decision | VERIFY EVERY MAP: the owner ruled the figure check runs EVERYWHERE (*"yes everywhere … mobs are placed ON TOP of the map, makes no sense that the picture has them"*), so the CLASSIC path gets the SAME vision check before storing, failing loudly by name and persisting nothing; a missing vision model is LOUD, never a silent skip. Cost: one vision call per map (approved).
...
IN-FLIGHT | row=340 | state=SUPERSEDED-BY-343 (OWNER: DID NOT FIX IT) | sha=ac9c1e8 | writer=34896b7f (retired) | model=deepseek-official/deepseek-flash | note=worktree/branch retired. THE OWNER REPORTS THE SCROLL IS STILL GONE: kept as a RED against its own claim, fix-forwarded as 343. Landed: ONE scroller = the body, `85vh` + `@supports` `min(85svh,85dvh)`, a pinned `scrollMargin`. OWED: the iPad check (jsdom cannot scroll), docs/18 SS5 (a base `@supports` cap beats a caller plain cap).
IN-FLIGHT | row=341 | state=DISPATCHED | writer=d7226a11-c007-4d75-b595-7feda5016fb4 | worktree=/home/administrator/projects/Campaigner/worktrees/map-classic | branch=map-classic | model=deepseek-official/deepseek-flash (inherited route) | note=the classic map path gets the figure check (owner: everywhere); brief `.gate-logs/briefs/row341.md`; reports ON LANDING or BLOCKED only.
IN-FLIGHT | row=343 | state=DISPATCHED | writer=698761ae-afe0-4ab7-b5c6-3a6922d9d305 | worktree=/home/administrator/projects/Campaigner/worktrees/scroll-fallback | branch=scroll-fallback | model=deepseek-official/deepseek-flash | note=fix-forward of 340: a DEFINITE dialog height (never `max-h`-only) so the WebKit flex child is bounded, ONE scroller, same for the shared base cap, STRUCTURE pins, and the trap in docs/05 + docs/17; brief `.gate-logs/briefs/row343.md`; the iPad check stays OWED.
