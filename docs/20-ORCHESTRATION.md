...
```
reconciled: f3aace6 · 2026-09-23T09:12Z (ROW 331 GATE GREEN — a player is never coverage-hidden by a veil or a fog, now ASSERTED (the behaviour was already right; the pin was missing). GATE GREEN: 375 files / 4812 tests, 7/7 chunks, lint 0, peak 2477MB, 598s. OPEN: QUEUE 311 (six owed pins), 318, 321, 330 (the ledger table-integrity pin); the level-9 encounter report stays closed-unreproducible.)
...
QUEUE | row RESERVED 330 | SMALL | THE DECISION LEDGER HAS NO TABLE-INTEGRITY PIN: 32 unescaped pipes inside code spans across 21 `docs/17` rows (one pipe in a TS union type is enough) render those rows with extra cells. Fix mechanically (escape only pipes INSIDE backtick spans, never a real separator) and land the pin with it — `one-board-rule.test.ts` is the precedent for the board.
QUEUE | row RESERVED 333 | IN FLIGHT — owner | BATTLE SPAWNING, TWO PARTS: (1) an "illustrate mobs with no image" checkbox on the spawn surface, enqueuing the EXISTING single-mob portrait for exactly the spawned creatures whose `creaturePortraitArt` is `none`; (2) author a fresh mob with the built-in `npc-smith` and spawn it — name + a STRUCTURED level bound to `entityLevelHint` (the authority; docs/17 row 289) + a description, and a failed or blockless result FAILS LOUDLY and spawns nothing.
QUEUE | row RESERVED 334 | IN FLIGHT — owner | THE PARTY WIZARD: two questions per character (name, initiative bonus) and a Done that ends the loop, for fast party onboarding. Reuses the ONE creation seam and `blankArtifactData('pc')` (HP 20, `statBlock: null` — the owner’s row-308 rule re-asserted at this entry point) and writes `initiativeOverride`; a blank name REFUSES and creates nothing.



IN-FLIGHT | row=334 | state=DISPATCHED | writer=2afc0bcf-3976-4c06-906e-0520b3b15204 | worktree=/home/administrator/projects/Campaigner/worktrees/party-wizard | branch=party-wizard | model=deepseek-official/deepseek-flash (inherited route) | note=the party wizard (2 questions/character, Done); brief `.gate-logs/briefs/row334.md`; reports ON LANDING or BLOCKED only.
IN-FLIGHT | row=333 | state=DISPATCHED | writer=36ba4a7b-d281-4ba7-87a9-ad02f7d6cc0c | worktree=/home/administrator/projects/Campaigner/worktrees/spawn-plus | branch=spawn-plus | model=deepseek-official/deepseek-flash (inherited route) | note=spawn checkbox + Smith-authored mob (part 1 lands first); brief `.gate-logs/briefs/row333.md`; reports ON LANDING or BLOCKED only.
