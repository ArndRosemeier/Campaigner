...
IN-FLIGHT | row=349 | state=DISPATCHED | writer=429458ce-8618-469a-b5f7-4f6fcadf4411 | note=OWNER CHOSE (A): heal name-only roster rows by COPYING the library stats onto them. DISPATCHER ERROR (verified): the brief and row 349 named `db/mobCopyRepair` as the seam to extend, but `89c55e8` DELETED that repair layer — the landing rides the seams that exist: `db/libraryCopy`, `domain/libraryCreature`, `battleRepo.healBattleBoardMap`. `initiativeBonus` is null for EVERY token, so the heal sets `artifactId`/`currentHp`.
