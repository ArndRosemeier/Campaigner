...
```
reconciled: d573103 · 2026-09-21T21:45Z (rows 305+306 gated as a UNION and row 307 gated on the tip; the owner then ruled that EVERY campaign player is in EVERY battle, always, with HP and no stat block required — reserved as row 308 and dispatched. NO writer in flight but 308's; the three landed writers are retired)
...
SESSION  | cos=session-b276401e-48d8-4c02-b2fb-7494264a77da | started=2026-09-20 | model=deepseek-official/deepseek-flash | state=LIVE (owner-designated 2026-09-21) | note=reconciled at `0b2b2b2`; landed rows 274, 275, 304, 305, 306 and 307, plus the AGENTS commit-pathspec guard (`07f227b`). Dispatched row 308 (every campaign player in every battle). Holds ONE standing goal, PAUSED; wake events only.
...
QUEUE | row RESERVED 308 | MEDIUM — DISPATCHED | EVERY CAMPAIGN PLAYER IN EVERY BATTLE (`ensurePcTokens` is the ONE idempotent seam; its INPUT at `db/fighterStats.ts:114` silently drops a statless PC, and a battle already `live` gains none on a plain open — one open-path trigger through the same seam). Plus the new-player HP default (20). Probe report `.gate-logs/discoveries/party-in-every-battle.md`.
