...
```
reconciled: 35e524d · 2026-09-21T23:57Z (DUPLICATE-FOLD ARC CYCLE 1: rows 313 `b14d5bb` (llm, 4 folds) + 312 `f0140fb` (pack/db/domain, 5 folds) landed; the UNION gate over `35e524d` is GATE GREEN — 373 files / 4775 tests, 7/7 chunks, lint 0, peak 2509MB, wall 584s — after ONE REAL RED: the name-key census counted `comparableName(` in `llm/encounterItems.ts` 3 where row 313 folded it to 2, fixed forward as `35e524d`. Baseline 13→4 groups. Cycle 2 = rows 315/316.)
...
QUEUE | row RESERVED 311 | SMALL — batched with the next code slice | FOUR OWED PINS, each found by an arm that stayed GREEN: a statless PC with a NON-ZERO `initiativeOverride` (row 308's pin asserts 0); the deep-link reload into a live board that misses a new player (docs/18 §5); row 309's IN-PLACE-FILL budget exemption; and row 313's two `on` delegates — the emitter arm reds only moduleGen's consumer, so `ChainRunner.on`/`RunEngine.on` dropping a listener would be silent.
QUEUE | row RESERVED 315 | cycle-2 writer A — DISPATCHED | folds G1 (two identical `Field` wrappers, `12c29e97676c2c29`), G6 (`workerCount` ×3, `7422a6200878f5a8`) and G9 (`onFetchProgress`/`onProgress`, `b54ed0a3f602f5cd`).
QUEUE | row RESERVED 316 | cycle-2 writer B — DISPATCHED | folds G2 (the three identical missing-entity panels, `1996f7df8ca0ab87`) — the LAST group in the `src/` baseline.

IN-FLIGHT | row=315 | state=DISPATCHED | writer=cdab90c9-f78a-4d4f-ae40-aca86c82afc5 | worktree=/home/administrator/projects/Campaigner/worktrees/dup-ui-a | branch=dup-ui-a | model=deepseek-official/deepseek-flash (inherited route) | note=cycle-2 writer A: G1 (two `Field` wrappers), G6 (`workerCount` ×3), G9 (`onFetchProgress`/`onProgress`); brief `.gate-logs/briefs/row315.md`; reports ON LANDING or BLOCKED only.
IN-FLIGHT | row=316 | state=DISPATCHED | writer=c78599fe-4946-483b-a75b-b72c61f86bd5 | worktree=/home/administrator/projects/Campaigner/worktrees/dup-ui-b | branch=dup-ui-b | model=deepseek-official/deepseek-flash (inherited route) | note=cycle-2 writer B: G2 (the three identical missing-entity panels), the LAST `src/` baseline group; brief `.gate-logs/briefs/row316.md`; reports ON LANDING or BLOCKED only.
...
LANDED | row=309 | sha=6bb3fac | verify=GATE GREEN — 373 files / 4775 tests, 7/7 chunks, 0 voided, lint 0, peak 2546MB, wall 625s (log `.gate-logs/gate-3-20260921T205535-800`). ASSERTED CAST IS ABSOLUTE: the draft transcribes `data.assertedCast:[{name,count}]` onto the EXISTING advisory channel + the row; presence required (repair then FAIL); asserted figures EXEMPT inside `checkRoomBudget`; substitutions cannot cover one.
LANDED | row=308 | sha=5074450 | verify=GATE GREEN — 372 files / 4758 tests, 7/7 chunks, 0 voided, lint 0, peak 2504MB/3000MB, wall 627s (log `.gate-logs/gate-3-20260921T201041-2642`). EVERY CAMPAIGN PLAYER IN EVERY BATTLE (statless included; new players default to HP 20; a pre-existing battle gains them on open). THE RED IS KEPT: the first gate over `c240053` failed LINT (`prefer-optional-chain` — whose suggested fix was a SEMANTIC bug), fixed forward as `5074450`.
LANDED | row=307 | sha=d573103 | verify=GATE GREEN — 372 files / 4748 tests, 7/7 chunks, 0 voided, lint 0, peak 2494MB/3000MB, wall 580s (log `.gate-logs/gate-3-20260921T192646-20365`). ONE `RUN_IMAGE_CANDIDATES = 1` feeds BOTH the illustrate and battlemap sites. Arm: the MAP call reverted to 2 → RED on 2 named pins. EVIDENCE GAP: the writer's arm hash `7006334e…` ≠ the landed `1c8d12a0…` (another revision); mine covers the landing.
LANDED | row=305 | sha=7a75312 | verify=GATE GREEN — the UNION gate over rows 305+306: 372 files / 4744 tests, 7/7 chunks, 0 voided, lint 0, peak 2522MB/3000MB, wall 581s (log `.gate-logs/gate-3-20260921T191436-18130`). The reader remembers its session scroll, module-keyed and in-memory. HONEST GAP: the `module.id === moduleId` half of the restore gate is UNPINNED — the dispatcher's own arm left the keying pin GREEN (inert in jsdom; needs a real browser, docs/18 §4).
LANDED | row=306 | sha=0fa5bb4 | verify=GATE GREEN — the dispatcher's integrated run over 0fa5bb4: 371 files / 4739 tests, 7/7 chunks, 0 voided, lint 0, peak 2576MB/3000MB, wall 577s (log `.gate-logs/gate-3-20260921T190406-24635`). Cure: `key={activeRunId}` on the per-run subtree; backstop: ONE `assertPickKeepsOwnCandidates` shared by BOTH pick paths. Dispatcher arm: the image half narrowed to the map label → RED on the named backstop pin, restored byte-identical.
```
