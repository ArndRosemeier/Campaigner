...
```
reconciled: 8ab8d88 · 2026-09-25T07:55Z (ROW 356 GATE GREEN — the adversarial pass is ONE callable seam, advisory, with exactly the owner's four kinds and the editor's guard split from its core; nothing calls it yet. AWAITING-OWNER `id=premise-undo`: the version stack EXCLUDES the premise, so a premise edit would be un-undoable — (A) extend it is recommended. 357 waits on that answer. OPEN: the iPad scroll check, the itemization go-ahead (347), the stylize retry (348), the owed pin 351.)
...
QUEUE | rows RESERVED 357-358 | ARC — ADVERSARIAL GENERATION (docs/17 rows 352/353/356): 357 = the automatic trigger INSIDE generation, PREMISE FIRST then each part (BLOCKED on `id=premise-undo`: the premise write needs the snapshot to cover it); 358 = the chat trigger via the `<change>` half.
...
AWAITING-OWNER | id=premise-undo | cost=1 slice | question=The version stack (your undo for AI edits) stores "every planned part, SPINE PREMISE EXCLUDED" (verified in moduleVersion.ts). So a premise edit by the pass CANNOT be undone through the versions menu. RECOMMENDED (A): extend that ONE seam to cover the premise, so the undo restores it. REJECTED: (B) a premise edit that stays un-undoable; (C) critique-only, which contradicts your "also on the premise". Go-ahead on (A)?
