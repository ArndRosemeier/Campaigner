...
LANDED | row=350 | sha=0baa891 | verify=the same union gate (row 349). TWO DECLARED SOURCE INVENTORIES GREW by `db/mobStatRepair.ts` — the `updateBattle(` façade population and the `matchesSlotLabel(` grammar population — and BOTH ARE USES, read at the call site before either pin was touched (single calls; through the façade, not around it; no second `RegExp`). Both extended, with the new caller's own assertions.
