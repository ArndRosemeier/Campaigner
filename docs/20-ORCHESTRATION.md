...
```
reconciled: 8ab8d88 · 2026-09-25T08:10Z (ROW 356 GATE GREEN — the pass is one advisory seam with exactly the owner's four kinds. THE OWNER CHOSE "make undoable": row 357 extends the ONE version snapshot to capture and restore the PREMISE, row 358 wires the pass into generation (premise first, then each part) — BOTH IN FLIGHT in separate worktrees. Row 359 (the chat trigger) is queued. OPEN: the iPad scroll check, the itemization go-ahead (347), the stylize retry (348), the owed pin 351.)
...
IN-FLIGHT | row=357 | state=DISPATCHED | writer=cada2446-f97e-44e8-af75-09252ffa72fc | worktree=/home/administrator/projects/Campaigner/worktrees/premise-undo | note=OWNER: "Please make undoable" — extend the ONE version snapshot so a version CAPTURES the module premise and the RESTORE puts it back (today `moduleVersion` says "every planned part, spine premise excluded", so a premise edit is un-undoable). Additive on read, no migration, no second seam. No generation change. Brief: row357.
IN-FLIGHT | row=358 | state=DISPATCHED | writer=ec4d8220-9059-469c-ab89-8003d7eb92c5 | worktree=/home/administrator/projects/Campaigner/worktrees/adversarial-trigger | note=THE TRIGGER: run the EXISTING pass inside generation when the row's flag is ON — PREMISE FIRST, then each PART as written — writing through the existing seams with the scaffolding intact, naming the sub-phases in the dock, containing a failure so the text stays UNCHANGED. Flag OFF must be BYTE-IDENTICAL. Brief: row358.
