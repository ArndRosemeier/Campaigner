...
```
reconciled: be6099f · 2026-09-26T12:30Z (ROWS 366+367 GATE GREEN — gallery favourites sort favourites-first (newest highest) with non-favourites untouched, and the chat-removal slice landed as a VERDICT: the existing Clear chat already deletes the saved conversation, so no twin button, only five new proof pins. OPEN: should the button SAY "Delete"? STILL HIS: Reset-to-default on the three personas, the iPad scroll check. QUEUED: 361, 363-364 (GM assist persist + context).)
LANDED | row=366 | sha=be6099f | verify=GATE GREEN — the gate over 366+367: 387 files / 4999 tests, lint 0. GALLERY FAVOURITES: ONE nullable timestamp on the image row carries both flag and order, a star per gallery image, favourites first (newest highest), non-favourites in EXACTLY today's order; sort only — no pick/delete/print path reads it. ARMS: flag ignored → 5 RED; ascending → 2 RED; non-favourites re-sorted → 4 RED.
