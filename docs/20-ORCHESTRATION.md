...
```
reconciled: 0baa891 · 2026-09-24T18:45Z (ROWS 349+350 GATE GREEN — spawned core mobs are repaired by COPY on battle open, and the two source inventories the new file entered are declared. NO WRITER IN FLIGHT. OPEN: the iPad scroll check (the only proof jsdom cannot give), the itemization go-ahead (347), the stylize retry (348), the owed pin 351.)
