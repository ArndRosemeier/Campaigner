...
QUEUE | row RESERVED 343 | IN FLIGHT — owner defect | THE SPAWN DIALOG *STILL* DOES NOT SCROLL on iPad with MANY SCREENS OF MOBS (340 did not cure it): content overflows, there is NO scrollbar, a drag does nothing. Cause (INFERRED, not device-measured): the scroller moved off the DIALOG — capped, scrolling its OWN content, which works on WebKit — onto a `flex-1 min-h-0` CHILD under a `max-height`-only parent WebKit does not bound, so it clips. Fix = a DEFINITE `h-…`, ONE scroller.
