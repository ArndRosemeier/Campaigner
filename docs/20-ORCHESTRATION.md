...
```
reconciled: 5c492d2 · 2026-09-24T11:50Z (ROW 346 GATE GREEN — the illustrator now RECEIVES the owner's direct instruction, which was dropped in silence, and ONE precedence clause makes it outrank the description; both dispatcher arms RED by name. AWAITING-OWNER: the itemization arc (row 347). QUEUED: row 348, the same silent drop on the map stylize RETRY. OWED: the owner's iPad check on the scroll fix — the one proof jsdom cannot give.)
