...
IN-FLIGHT | row=349 | state=DISPATCHED | writer=429458ce-8618-469a-b5f7-4f6fcadf4411 | note=OWNER CHOSE (A): heal name-only roster rows by COPYING the library creature's stats onto them via the EXISTING `db/mobCopyRepair` seam. The defect: the clean-cut purge turned a library CITATION into `{ type: 'none' }` (`cleanCut.ts:94-96`) → `statBlock: null`, so it spawns statless (no initiative, no damage). Must equal `buildMobPickEntry`'s entry, stay idempotent, NAME an unresolvable row. Brief: row349.
