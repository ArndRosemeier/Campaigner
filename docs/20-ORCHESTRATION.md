...
```
reconciled: 4ca8515 · 2026-09-23T00:15Z (ROWS 327+328 GATE GREEN over the union — module-owned rows are selectable and exportable for reuse, and a battle adopts the encounter’s current map (heal when it has none, an explicit action when they differ, and the live-skip toast now names that action). GATE GREEN: 375 files / 4809 tests, 7/7 chunks, lint 0, peak 2491MB, 628s. OPEN: QUEUE 311 (six owed pins), 318, 321; the level-9 encounter report stays closed-unreproducible.)
...
QUEUE | row RESERVED 331 | IN FLIGHT — owner | ASSERT A PLAYER IS NEVER COVERAGE-HIDDEN: the rule already exists (`use-battle.coveredTokenIds` scopes to kind `npc`; the surface drops only those in player-safe view; veils paint before tokens), but NO test references the predicate — so the deliverable is the missing pin, in BOTH veil kinds, plus the layer-order pin. Owner: *"please assert that players on the battlemaps are not covered by veils or fog."*


IN-FLIGHT | row=331 | state=DISPATCHED | writer=c44df2c2-cf9f-4984-849c-66fc3c11a96a | worktree=/home/administrator/projects/Campaigner/worktrees/pc-visible | branch=pc-visible | model=deepseek-official/deepseek-flash (inherited route) | note=pin the PC-never-coverage-hidden guarantee (test-only unless proven wrong); brief `.gate-logs/briefs/row331.md`; reports ON LANDING or BLOCKED only.
