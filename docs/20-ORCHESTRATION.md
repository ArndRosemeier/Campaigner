...
```
reconciled: 381aa6e · 2026-09-15T10:05Z
...
IN-FLIGHT | row=171 | branch=feat/pack-array-docs | worktree=/tmp/campaigner-arraydocs | base=381aa6e | writer=session-79c24c1d-b9d4-41ee-b292-05aad144bca9 | state=running | note=OWNER DECISION A: a top-level array of pack documents unwraps into N documents in the ONE parse seam, and the SEVEN identical `isRecord` copies (one per adapter) fold into ONE shared predicate — the owner's own instruction: "keep in mind centralization … if there are 5 places that get affected, consider if its possible to make that just 1 place"
...
QUEUE | source=docs/18 §5 known debt, each verified at HEAD + docs/19 §11's remaining build order | note=the owner-decided array-document slice is in flight (row 171); the debted `lib/pdfExport.statBlockSection` (the third consumer of the text→blocks rule) is the next slice, un-briefed until its ~20 `labelValue` call sites are scoped; docs/19 §11 slice 6 (print refinement / duplex pairing) is owner-DEFERRED; the Advanced floor editor's minimum and the Cartographer's brief parity are recorded debts with a stated reason not to touch them
