...
QUEUE | row RESERVED 360 | ARC — ADVERSARIAL GENERATION (docs/17 rows 352/353/356, owner-amended): the CHAT TRIGGER — extend the existing `<change>` command half so a chat turn can run the pass on the premise or a part and render the outcome card the chat already has.
