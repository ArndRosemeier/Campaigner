...
```
reconciled: b863efd · 2026-09-24T10:25Z (ROW 341 LANDED AND RETIRED — its SHA is on origin/main, the dispatcher's OWN arm is RED by name, worktree/branch/session retired. IN FLIGHT: row 343, the iPad scroll fix-forward, whose cause is CONFIRMED in-repo — `HelpDialog` scrolls the same inner body because its dialog is a DEFINITE height, while the picker is `max-height`-only. OWED: ONE full union gate over 340+341+343 once 343 lands, and the owner's iPad check, which is the only proof.)
...
QUEUE | row RESERVED 344 | SMALL — batched with the next code slice | THE OTHER TWO DIALOGS WITH THE BROKEN SCROLL SHAPE, if row 343 does not fold them: `SetupWizardDialog.tsx:156` (the ONBOARDING path — severe) and `peek-modal.tsx:103` share SpawnPicker's `flex max-h-[85vh] flex-col overflow-hidden` + inner `min-h-0 flex-1 overflow-y-auto`, while `HelpDialog` scrolls because its dialog is a DEFINITE `h-[80vh]`. The owner reported only the picker.
...
IN-FLIGHT | row=343 | state=DISPATCHED | writer=698761ae-afe0-4ab7-b5c6-3a6922d9d305 | worktree=/home/administrator/projects/Campaigner/worktrees/scroll-fallback | branch=scroll-fallback | model=deepseek-official/deepseek-flash | note=fix-forward of 340: a HARD definite height (the HelpDialog control) via ONE shared seam, the BASE cap unchanged, at 3 dialogs with an "exactly one" pin and the trap in docs/05 + docs/17; brief `.gate-logs/briefs/row343.md`; the iPad check stays OWED.
LANDED | row=341 | sha=a018460 | verify=DISPATCHER ARM RED BY NAME — removing the classic figure check made the NAMED classic-refusal pin fail (`expected 'completed' to be 'failed'`); pristine/restored `runEngine.ts` hashes match (`3272154a…`). THE FULL UNION GATE (340+341+343) IS OWED. ONE client factory serves BOTH map paths; the check runs on the RAW blob BEFORE intake/store; a vision failure propagates. HONEST LIMIT: a vision read is a MODEL judgement, not a pixel proof.
