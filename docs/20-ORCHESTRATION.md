...
QUEUE | row RESERVED 340 | IN FLIGHT — owner defect | THE SPAWN DIALOG NO LONGER SCROLLS on iPad (a regression from rows 336/339): TWO nested scrollers plus a `vh` cap that is the LARGE viewport on iOS. Fix = ONE scroller (the body) with the virtualizer scrolling it, and a small-visible-viewport cap (`svh` first, `vh` fallback, `dvh` override). The class merge was CHECKED and is NOT the cause. Real-device check owed.
...
IN-FLIGHT | row=340 | state=LANDED-PENDING-GATE | sha=ac9c1e8 | writer=34896b7f (retired) | model=deepseek-official/deepseek-flash | note=the scroll fix landed (ONE scroller = the body; `85vh` + `@supports` `min(85svh,85dvh)`; a `scrollMargin` consequence found and pinned) and its worktree/branch are RETIRED; the union gate with 341 is pending. OWED: the owner iPad check (jsdom cannot scroll) and the docs/18 SS5 debt — a base `@supports` cap beats a caller plain cap.
