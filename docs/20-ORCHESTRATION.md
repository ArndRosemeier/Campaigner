...
```
reconciled: 68d9e37 · 2026-09-26T06:35Z (ROWS 362+365 GATE GREEN. GM assist slice 1: a second chat NEXT TO the module chat, STORY-side, on the ONE pipeline; thread session-only until 363. Row 365: NPC/location/faction descriptions carry DETAIL, not story — BUT the owner must Reset to default those three personas (they are seed-once) for it to reach his campaign. QUEUED 361: an unasserted textless-premise refusal. OPEN: the iPad check, the itemization go-ahead (347), the stylize retry (348).)
IN-FLIGHT | row=366 | state=DISPATCHED | writer=fd3f369d-da5b-4625-ace4-6e1c67358b3e | worktree=/home/administrator/projects/Campaigner/worktrees/gallery-favourites | note=GALLERY FAVOURITES (owner: a SORT only — favourites on top, newest highest): ONE nullable timestamp on the image row carries both the flag and the order, a star per gallery image, non-favourites in EXACTLY today's order. NOT a pick preference, NOT cleanup protection, NOT a print filter. Brief: row366.
IN-FLIGHT | row=367 | state=DISPATCHED | writer=cd9632c3-f0fb-430c-99b1-adc7d3b6f4ac | worktree=/home/administrator/projects/Campaigner/worktrees/chat-remove | note=REMOVE A WHOLE CONVERSATION (owner: delete a whole conversation, module chat AND GM assist): MUST FIRST settle what the EXISTING Clear deletes — if it removes everything that is a BLOCKED, not a twin button; if it leaves a visible record, THAT is the defect. Removing one conversation leaves the OTHER byte-identical.
...
LANDED | row=346 | sha=5c492d2 | verify=GATE GREEN — 381 files / 4874 tests, lint 0. THE ILLUSTRATOR PRIORITISES THE DIRECT INSTRUCTION: his `Additional instruction:` was DROPPED SILENTLY (the step passed the raw, EMPTY `extraInstruction`); it now reads the ONE composer `directInstructionFor`, and ONE `IMAGE_DIRECT_INSTRUCTION_PRECEDENCE_CLAUSE` ties to the instruction in both branches, emitting NOTHING when none is set. ARMS: clause dropped → 6 RED; reach reverted → 2 RED.
...
LANDED | row=349 | sha=7df1f6f | verify=GATE GREEN — 382 files / 4885 tests, lint 0. CORE MOBS ARE STATFUL: the repair COPIES library stats onto the name-only rows the purge left and backs the frozen stateless tokens with a seed row + HP, on battle open (ONE trigger). ARMS: name resolution → 9 RED, statful guard → 1 RED, token HP patch → GREEN = A REAL PIN GAP (stored HP asserted by nothing), owed as 351. Also fixed: a second spawn of a frozen creature read stateless at HEAD.
LANDED | row=350 | sha=0baa891 | verify=GATE GREEN — the SAME union run as row 349, whose tree already contained this fix-forward (it was committed before that gate ran). TWO DECLARED SOURCE INVENTORIES GREW by `db/mobStatRepair.ts` — the `updateBattle(` façade population and the `matchesSlotLabel(` grammar population — and BOTH ARE USES, read at the call site before either pin was touched (single calls; through the façade, not around it; no second `RegExp`).
...
LANDED | row=354 | sha=2c675e3 | verify=GATE GREEN — 382 files / 4893 tests, lint 0. SLICE 1 of the adversarial-generation arc: the OFF-by-default module-row flag + data path (draft, checkbox, NewModule, createModule) — no pass/critique/editor/model call. ARMS: schema default flipped → 2 RED; `createModule` ignoring its input → 4 RED; the spine made flag-dependent → the BYTE-IDENTICAL pin RED. DEVIATION ACCEPTED: the draft field defaults, so a pre-slice draft parses.
LANDED | row=355 | sha=93834fb | verify=GATE GREEN — the same union run as row 354, whose tree already contained it. THE FIRST UNION GATE OVER 354 WAS RED ON THE DISPATCHER'S OWN BOOKKEEPING: docs/20 measured 141 against the 140 cap because the check used `grep -c ''` while the pin measures `split('\n').length` (one MORE when the file ends in a newline). Fixed by pruning padding; the pre-commit guard now uses the PIN's measure. Board only.
LANDED | row=360 | sha=7331f52 | verify=GATE GREEN — 383 files / 4950 tests, lint 0. The LAST slice: the ONE `<change>` seam gained a second shape (`adversarial="premise"` / `adversarial="part" part="N"`), so a chat turn runs the UNCHANGED pass on the premise or a part and renders the CRITIQUE FINDINGS with the edit in existing chat surfaces; accepting rides the existing applier + split-save (undoable); an empty critique is quiet. ARMS: findings dropped → 2 RED; premise write bypassed → 2 RED.
...
LANDED | row=362 | sha=27359aa | verify=GATE GREEN — 385 files / 4967 tests. GM ASSIST SLICE 1: the chat framing is a PARAMETER of the ONE pipeline (one builder, default module), plus ONE `GM_ASSIST_FRAMING` (STORY-side, default = 2-4 ideas) and a second chat surface on its own store key; the module prompt AND both payloads held to a GOLDEN from before the change; edits ride the same applier. ARMS: framing ignored → 2 RED; shared store key → 2 RED; persistence gate forced → 2 RED.
LANDED | row=365 | sha=68d9e37 | verify=GATE GREEN — 386 files / 4984 tests, lint 0. NPC/LOCATION/FACTION DESCRIPTIONS CARRY DETAIL, NOT STORY: ONE shared clause on exactly those three personas, ADDITIVE (grounding stays), the rest BYTE-FROZEN (the owner chose that scope); arms 3 red by name. ⚠ OWNER ACTION: personas are SEED-ONCE, so his EXISTING campaign keeps the OLD prompt — Settings → Personas → Reset to default on those three, then regenerate (the mechanism he chose at row 140).
...
LANDED | row=356 | sha=8ab8d88 | verify=GATE GREEN — 383 files / 4909 tests. THE PASS AS ONE SEAM: a zod critique whose kinds are EXACTLY the owner's four (a fifth is REJECTED), ADVISORY (findings never block; an empty critique is quiet and never calls the editor), then the editor through the SAME core the canvas uses, GUARD SPLIT so the pass can run inside generation; snapshot FIRST; nothing calls it yet. ARMS: fifth kind → 2 RED; quiet path cut → 1 RED; canvas guard off → 1 RED.
LANDED | row=357 | sha=32c8d57 | verify=GATE GREEN — the union gate over 357+358: 383 files / 4933 tests, lint 0. THE PREMISE IS UNDOABLE: the ONE version row carries `premise` (nullable, additive — a pre-slice row parses as "not captured" and restores exactly as before), captured in the SAME read as the parts, applied BEFORE any part write, failing LOUDLY rather than half-restoring; the menu states each entry's premise half. ARMS: capture dropped → 3 RED; restore dropped → 2 RED.
LANDED | row=358 | sha=b09e089 | verify=GATE GREEN — the same union run. TRIGGER: with the row's flag ON the EXISTING pass runs INSIDE generation at two sites: the PREMISE before any part prompt, then each PART as it is written — writing through the existing seams, naming the sub-phases in the dock, passing the abort signal, containing a failure. FLAG OFF IS BYTE-IDENTICAL (a golden transcript from the pre-change tree). ARM: the flag ignored → 21 RED incl. that golden.
LANDED | row=359 | sha=498f1b1 | verify=GATE GREEN — the same union run already contained this commit (the gate ran after it), recorded HERE because without it the badge counted these two SOURCE files as unverified. TWO STALE COMMENTS FIXED FORWARD, both reported by writers rather than absorbed: the row-356 "KNOWN BOUNDARY" that row 357 turned into a lie about the snapshot, and the premise-origin list that said "two writers" while listing three and now has four. Comment-only.
